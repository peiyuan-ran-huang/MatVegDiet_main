...
# This script is to perform negative control outcome analysis on breastfeeding (with imputed data) for vegetarian diets (in subgroups) in BiB.
...
for (i in 1:dat_imp$m) {
  mod_2[[i]] <-
    clm(
      bf_dur_4c ~ VegDiet_3cat + age_Mat_con + ethnic_Mat_bin + edu_Mat_3cat  + IMD_Fam_cat + parity_Mat_bin + BMI_Mat_PRE.p_con + smoking_Mat_EAR.p_bin + alcohol_Mat_EAR.p_bin + sex_Chi_bin,
      data = dat[[i]]
    )
}
...
for (i in 1:dat_imp$m) {
  mod_3[[i]] <-
    clm(
      bf_dur_4c ~ VegDiet_3cat + age_Mat_con + ethnic_Mat_bin + edu_Mat_3cat  + IMD_Fam_cat + parity_Mat_bin + BMI_Mat_PRE.p_con + smoking_Mat_EAR.p_bin + alcohol_Mat_EAR.p_bin + sex_Chi_bin + any.supp_Mat_EAR.p_bin,
      data = dat[[i]]
    )
}
...
write.xlsx(
  obs.tbl_VegDiet_ord,
  "results/BiB/IMP_BF_obs.tbl_VegDiet.subgroup_ord.xlsx",
  overwrite = T
)
obs.tbl_VegDiet_ord <-
  read.xlsx("results/BiB/IMP_BF_obs.tbl_VegDiet.subgroup_ord.xlsx")
...
write.xlsx(
  obs.res_VegDiet_ord,
  "results/BiB/IMP_BF_obs.res_VegDiet.subgroup_ord.xlsx",
  overwrite = T
)
obs.res_VegDiet_ord <-
  read.xlsx("results/BiB/IMP_BF_obs.res_VegDiet.subgroup_ord.xlsx")
...
for (i in 1:dat_imp$m) {
  mod[[i]] <- glm(
    VegDiet_3cat ~ IMD_Fam_cat + age_Mat_con + ethnic_Mat_bin,
    data = subset(
      dat[[i]],
      VegDiet_3cat %in% c("Non-vegetarian", "Pesco-vegetarian")
    ),
...
write.xlsx(
  obs.res_VegDiet_SEP,
  "results/BiB/IMP_BF_obs.res_VegDiet.subgroup_SEP.xlsx",
  overwrite = T
)
obs.res_VegDiet_SEP <-
  read.xlsx("results/BiB/IMP_BF_obs.res_VegDiet.subgroup_SEP.xlsx")
...
obs.forest_VegDiet_SEP <- ggforestplot::forestplot(
  ...
  shape = Outcome,
  xlab = "OR and 95% CI",
  title = "Effect of socioeconomic position in BiB",
  logodds = T
) +
...
ggsave(
  obs.forest_VegDiet_SEP,
  file = "results/BiB/IMP_BF_obs.forest_VegDiet.subgroup_SEP.png",
  height = 3,
  width = 9
)
