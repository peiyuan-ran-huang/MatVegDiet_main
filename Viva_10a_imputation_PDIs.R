...
# This script is to perform multiple imputation by chained equations (MICE) for plant-based diet indices (PDIs) in Project Viva.
...
# Multiple imputation by chained equations (MICE) - Only apply to covariates
# [NOTE: Make sure the variables used as predictors (of other variables to be imputed)]
#        are either complete in the dataset or being imputed as well;
#        otherwise, the imputation will not work properly.]

## Load selected variables
substantive_var <-
  read.xlsx("data/Viva/select_var.xlsx", sheet = "Substantive_PDIs")
...
auxiliary_var <-
  read.xlsx("data/Viva/select_var.xlsx", sheet = "Auxiliary_PDIs")
...
other_imp <-
  c(
    "NON.iron.multivit.supp_Mat_EAR.p_bin",
    "energy_Mat_EAR.p_con",
    "energy_Mat_MID.p_con"
  )  # Other variables to be imputed (e.g., used for adjustment)
other_N_imp <-
  c(
    "iron.multivit.supp_Mat_EAR.p_bin",
    ...
    "uPDIm_2_z",
    "uPDIm_z"
  )  # Other variables NOT to be imputed (e.g., used as exposure, to avoid collinearity, for stratification, or as alternative exposure)
################################################################################
...
dat <- convert_binary_factors(dat)  # Convert binary factors (1/2) to 0/1
...
method[c("aid",
         ...
         Viva_out_con,
         Viva_out_cat,
         other_N_imp)] <- ""  # IDs, exposures, outcomes, other non-imputed variables - NOT being imputed
method
...
high_corr_vars <- findCorrelation(corr_matrix, cutoff = 0.9, names = T)
cat("Highly correlated variables (cutoff = 0.9):",
    high_corr_vars,
    "\n")  # hPDI uPDI_z hPDI_2 hPDI_1 PDI_z uPDI_2_z uPDI_1 energy_Mat_DUR.p_con PDI_1 PDI_2_z - OK

## Set predictor matrix for imputation
predictorMatrix <- make.predictorMatrix(dat)
predictorMatrix[, c("aid", Viva_out_bin, Viva_out_con, Viva_out_cat, other_N_imp)] <-
  0  # NOT being used in imputation for other variables: IDs, outcomes, other non-imputed variables
predictorMatrix[c("aid",
                  "PDI",
                  "hPDI",
                  "uPDI",
                  Viva_out_bin,
...
dat_cov_md.pattern <- md.pattern(dat_cov)  # Check missing data patterns
summary(dat_cov_md.pattern)

num_incomplete_cases <- sum(!complete.cases(dat_cov))
num_total_cases <- nrow(dat_cov)
(num_incomplete_cases / num_total_cases) * 100  # Percentage of incomplete cases: 13.51496%
...
## Save imputed data
save(dat_imp, file = "data/Viva/dat_exp_cov_out_IMP_PDIs.RData")
...
dat_imp_long <- dat_imp_long[, !grepl("^.imp|^\\.id", colnames(dat_imp_long))]
head(dat_imp_long)
dim(dat_imp_long)  # 56160    65
...
dim(dat)  # 1872   66
dim(dat_imp_long)  # 56160    66

## Combine observed and imputed data
dat_mix <- rbind(dat, dat_imp_long)
dat_mix$marker <- factor(dat_mix$marker, levels = c("Observed", "Imputed"))
head(dat_mix)
dim(dat_mix)  # 58032    66
...
tab_ori_imp <- tbl_summary(
  ...
  modify_caption(
    "Distribution of covariates (for main analysis) in the observed and imputed datasets in Project Viva"
  ) %>%
  bold_labels() %>%
  modify_table_body(~ .x %>%
                      dplyr::mutate(across(
...
perc_imputed_df$variable_row_type <- paste0(perc_imputed_df$variable, "_label")
perc_imputed_df <- perc_imputed_df %>% select(-variable)
perc_imputed_df

tab_ori_imp_body <- as_tibble(tab_ori_imp$table_body)
tab_ori_imp_body$variable_row_type <- paste0(tab_ori_imp_body$variable, "_", tab_ori_imp_body$row_type)
tab_ori_imp_body <- tab_ori_imp_body %>% left_join(perc_imputed_df, by = "variable_row_type")
...
tab_ori_imp %>% as_flex_table() %>% flextable::save_as_docx(., path =
                                                              "results/Viva/00c-maternal_covariates_OBS.vs.IMP_PDIs.docx")
