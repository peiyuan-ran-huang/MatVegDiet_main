...
## Subset samples
dat_iron_0 <-
  subset(dat_1, iron.supp.freq_Mat_EAR.p_cat == "No use")
dat_iron_1 <-
  subset(dat_1, iron.supp.freq_Mat_EAR.p_cat == "Less than daily")
dat_iron_2 <-
  subset(dat_1, iron.supp.freq_Mat_EAR.p_cat == "Daily")
...
for (i in 1:dat_imp$m) {
  mod_iron_0[[i]] <-
    glm(
      ...
      data = subset(dat[[i]], iron.supp.freq_Mat_EAR.p_cat == "No use"),
      family = binomial
    )
}
...
for (i in 1:dat_imp$m) {
  mod_iron_1[[i]] <-
    glm(
      anaemia_preg_subsamp ~ VegDiet_bin + age_Mat_con + ethnic_Mat_bin + edu_Mat_3cat  + income_Fam_3cat + parity_Mat_bin + BMI_Mat_PRE.p_con + smoking_Mat_EAR.p_bin + alcohol_Mat_EAR.p_bin + sex_Chi_bin + NON.iron.supp_Mat_EAR.p_bin + energy_Mat_DUR.p_con + hPDIm,
      data = subset(dat[[i]], iron.supp.freq_Mat_EAR.p_cat == "Less than daily"),
      family = binomial
    )
}
...
for (i in 1:dat_imp$m) {
  mod_iron_2[[i]] <-
    glm(
      anaemia_preg_subsamp ~ VegDiet_bin + age_Mat_con + ethnic_Mat_bin + edu_Mat_3cat  + income_Fam_3cat + parity_Mat_bin + BMI_Mat_PRE.p_con + smoking_Mat_EAR.p_bin + alcohol_Mat_EAR.p_bin + sex_Chi_bin + NON.iron.supp_Mat_EAR.p_bin + energy_Mat_DUR.p_con + hPDIm,
      data = subset(dat[[i]], iron.supp.freq_Mat_EAR.p_cat == "Daily"),
      family = binomial
    )
}
...
write.xlsx(
  obs.res_VegDiet_bin,
  "results/IMP_STRAT.iron.freq_obs.res_VegDiet_bin.xlsx",
  overwrite = T
)
...
ggsave(
  obs.forest_VegDiet_bin,
  file = "results/IMP_STRAT.iron.freq_obs.forest_VegDiet_bin.png",
  height = 3,
  width = 6
)
