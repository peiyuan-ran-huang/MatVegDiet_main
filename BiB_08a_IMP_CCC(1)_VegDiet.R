...
## Part 1: Comparison between White British and South Asians
...
for (i in 1:dat_imp$m) {
  ...
  score <- rowSums(
    cbind(
      dat_WB[[i]]$smoking_Mat_EAR.p_bin == 0,
      dat_WB[[i]]$alcohol_Mat_EAR.p_bin == 0,
      dat_WB[[i]]$any.supp_Mat_EAR.p_bin == 1,
      dat_WB[[i]]$BMI_Mat_PRE.p_con >= 18.5 &
        dat_WB[[i]]$BMI_Mat_PRE.p_con < 25
    ),
    na.rm = F
  )
  dat_WB[[i]]$conscious_Mat_ord <- factor(score, levels = 0:4, ordered = T)
  var_lab(dat_WB[[i]]$conscious_Mat_ord) <- "Maternal composite lifestyle index (0–4)"
}
...
for (i in 1:dat_imp$m) {
  ...
  score <- rowSums(
    cbind(
      dat_SA[[i]]$smoking_Mat_EAR.p_bin == 0,
      dat_SA[[i]]$alcohol_Mat_EAR.p_bin == 0,
      dat_SA[[i]]$any.supp_Mat_EAR.p_bin == 1,
      dat_SA[[i]]$BMI_Mat_PRE.p_con >= 18.5 &
        dat_SA[[i]]$BMI_Mat_PRE.p_con < 25
    ),
    na.rm = F
  )
  dat_SA[[i]]$conscious_Mat_ord <- factor(score, levels = 0:4, ordered = T)
  var_lab(dat_SA[[i]]$conscious_Mat_ord) <- "Maternal composite lifestyle index (0–4)"
}
################################################################################
...
for (myoutcome in BiB_out_bin) {
  ...
  x <-
    cbind(outcome_x, N_x_WB, x_1, N_x_SA, x_2)  # Columns: Outcome name | N | Model 1 | N | Model 2; rows: Non-vegetarian | Pesco-/full vegetarian
  
  outcome_col <- as.data.frame(rep(outcome, 2))
  colnames(outcome_col) <- "Outcome"
  model_col <-
    as.data.frame(c("White British in BiB", "South Asian in BiB"))
  colnames(model_col) <- "Ethnicity"
  y <- cbind(outcome_col, model_col, rbind(y_1, y_2))
...
obs.tbl_VegDiet_bin <-
  obs.tbl_VegDiet_bin[, c(1, 3, 2, 4:6, 8, 7, 9:ncol(obs.tbl_VegDiet_bin))]  # For convenience when making tables
obs.tbl_VegDiet_bin
dim(obs.tbl_VegDiet_bin) # 19 outcomes * 2 categories = 38 obs in 2 models
...
for (i in 1:nrow(obs.res_VegDiet_bin)) {
  if (obs.res_VegDiet_bin$Ethnicity[i] == "White British in BiB") {
  ...
  } else if (obs.res_VegDiet_bin$Ethnicity[i] == "South Asian in BiB") {
    obs.res_VegDiet_bin$N_exp[i] <-
      obs.tbl_VegDiet_bin[obs.tbl_VegDiet_bin$Outcome == obs.res_VegDiet_bin$Outcome[i] &
...
obs.res_VegDiet_bin
dim(obs.res_VegDiet_bin)  # 19 outcomes * 1 category * 2 models = 38 obs
...
################################################################################
### Add in main meta-analysis results
metagen_obs.tbl_VegDiet_bin_MAIN <- read.xlsx("results/ALL/IMP_MAIN_meta.tbl_VegDiet_bin.xlsx")
metagen_obs.tbl_VegDiet_bin_MAIN$Population <- "Main results (all studies)"

obs.res_VegDiet_bin$Population <- obs.res_VegDiet_bin$Ethnicity

obs.res_VegDiet_bin <- rbind(obs.res_VegDiet_bin[, c("Outcome", "Population", "b", "se", "pval")], metagen_obs.tbl_VegDiet_bin_MAIN[, c("Outcome", "Population", "b", "se", "pval")])

obs.res_VegDiet_bin
################################################################################
### Add in whole BiB sample results
...
obs.res_VegDiet_bin_ALL$Population <- "Whole sample in BiB"

obs.res_VegDiet_bin <- rbind(obs.res_VegDiet_bin, obs.res_VegDiet_bin_ALL[, c("Outcome", "Population", "b", "se", "pval")])
obs.res_VegDiet_bin
################################################################################
...
obs.res_VegDiet_bin$Population <-
  factor(
    obs.res_VegDiet_bin$Population,
    levels = c(
      "South Asian in BiB",
      "White British in BiB",
      "Whole sample in BiB",
      "Main results (all studies)"
    )
  )
...
dim(obs.res_VegDiet_bin)  # (19 outcomes * 1 category * 2 models) + 21 (from main meta-analysis) + 19 (from whole BiB sample) = 78 obs
...
head(obs.res_VegDiet_bin)
dim(obs.res_VegDiet_bin)  # 78 -> 64 obs
################################################################################
### Remove if results only available in one ethnicity for any outcome and exposure
obs.res_VegDiet_bin <- obs.res_VegDiet_bin %>% group_by(Outcome) %>% filter(n() == 4) %>% ungroup()
head(obs.res_VegDiet_bin)
dim(obs.res_VegDiet_bin)  # 64 -> 40 obs
################################################################################

### Nightingale forest plots - !!! Wait to add in breastfeeding results; do continuous outcomes first !!!
...
obs.tbl_VegDiet_con[, 8] <-
  " "  # For convenience when making tables
obs.tbl_VegDiet_con <-
  obs.tbl_VegDiet_con[, c(1, 3, 2, 4:6, 8, 7, 9:ncol(obs.tbl_VegDiet_con))]  # For convenience when making tables
obs.tbl_VegDiet_con
...
for (i in 1:nrow(obs.res_VegDiet_con)) {
  if (obs.res_VegDiet_con$Ethnicity[i] == "White British in BiB") {
  ...
  } else if (obs.res_VegDiet_con$Ethnicity[i] == "South Asian in BiB") {
    obs.res_VegDiet_con$N_exp[i] <-
      obs.tbl_VegDiet_con[obs.tbl_VegDiet_con$Outcome == obs.res_VegDiet_con$Outcome[i] &
...
obs.res_VegDiet_con
dim(obs.res_VegDiet_con)  # 4 outcomes * 1 category * 2 models = 8 obs.
...
################################################################################
### Add in main meta-analysis results
metagen_obs.tbl_VegDiet_con_MAIN <- read.xlsx("results/ALL/IMP_MAIN_meta.tbl_VegDiet_con.xlsx")
metagen_obs.tbl_VegDiet_con_MAIN$Population <- "Main results (all studies)"

obs.res_VegDiet_con$Population <- obs.res_VegDiet_con$Ethnicity
obs.res_VegDiet_con <- rbind(obs.res_VegDiet_con[, c("Outcome", "Population", "b", "se", "pval")], metagen_obs.tbl_VegDiet_con_MAIN[, c("Outcome", "Population", "b", "se", "pval")])
obs.res_VegDiet_con
################################################################################
### Add in whole BiB sample results
...
obs.res_VegDiet_con_ALL$Population <- "Whole sample in BiB"

obs.res_VegDiet_con <- rbind(obs.res_VegDiet_con, obs.res_VegDiet_con_ALL[, c("Outcome", "Population", "b", "se", "pval")])
obs.res_VegDiet_con
################################################################################
...
obs.res_VegDiet_con$Population <-
  factor(
    obs.res_VegDiet_con$Population,
    levels = c(
      "South Asian in BiB",
...
dim(obs.res_VegDiet_con)  # (4 outcomes * 1 category * 2 models) + 4 (from main meta-analysis) + 4 (from whole BiB sample) = 16 obs.
...
ggsave(
  obs.forest_VegDiet_con,
  file = "results/BiB/IMP_CCC_obs.forest_VegDiet_con.png",
  height = 4.5,
  width = 8
)
...
obs.tbl_VegDiet_ord[, 8] <-
  " "  # For convenience when making tables
obs.tbl_VegDiet_ord <-
  obs.tbl_VegDiet_ord[, c(1, 3, 2, 4:6, 8, 7, 9:ncol(obs.tbl_VegDiet_ord))]  # For convenience when making tables
obs.tbl_VegDiet_ord
...
for (i in 1:nrow(obs.res_VegDiet_ord)) {
  if (obs.res_VegDiet_ord$Ethnicity[i] == "White British in BiB") {
  ...
  } else if (obs.res_VegDiet_ord$Ethnicity[i] == "South Asian in BiB") {
    obs.res_VegDiet_ord$N_exp[i] <-
      obs.tbl_VegDiet_ord[obs.tbl_VegDiet_ord$Outcome == obs.res_VegDiet_ord$Outcome[i] &
...
obs.res_VegDiet_ord
dim(obs.res_VegDiet_ord)  # 1 outcome * 1 category * 2 models = 2 obs.
...
################################################################################
### Add in main meta-analysis results
metagen_obs.tbl_VegDiet_ord_MAIN <- read.xlsx("results/ALL/IMP_BF_meta.tbl_VegDiet_ord.xlsx")
metagen_obs.tbl_VegDiet_ord_MAIN$Population <- "Main results (all studies)"

obs.res_VegDiet_ord$Population <- obs.res_VegDiet_ord$Ethnicity

obs.res_VegDiet_ord <- rbind(obs.res_VegDiet_ord[, c("Outcome", "Population", "b", "se", "pval")], metagen_obs.tbl_VegDiet_ord_MAIN[, c("Outcome", "Population", "b", "se", "pval")])
...
obs.res_VegDiet_ord_ALL$Population <- "Whole sample in BiB"

obs.res_VegDiet_ord <- rbind(obs.res_VegDiet_ord, obs.res_VegDiet_ord_ALL[, c("Outcome", "Population", "b", "se", "pval")])
obs.res_VegDiet_ord
################################################################################
...
obs.res_VegDiet_ord$Population <-
  factor(
    obs.res_VegDiet_ord$Population,
    levels = c(
      "South Asian in BiB",
...
dim(obs.res_VegDiet_ord)  # (1 outcome * 1 category * 2 models) + 1 (from main meta-analysis) + 1 (from whole BiB sample)  = 4 obs.
...
ggsave(
  obs.forest_VegDiet_bin_ord,
  file = "results/BiB/IMP_CCC_obs.forest_VegDiet_bin_ord.png",
  height = 9,
  width = 8
)
...
#   subset(obs.res_VegDiet_bin, select = c(Outcome, b, se, pval)),
#   subset(obs.res_VegDiet_con, select = c(Outcome, b, se, pval)),
#   subset(obs.res_VegDiet_ord, select = c(Outcome, b, se, pval))
# )
# obs.res_VegDiet_bin_con_ord
# 
# # Perform Z-test
# CCC_Z.test_bin_con_ord <- c()
# 
# for (my_out in unique(obs.res_VegDiet_bin_con_ord$Outcome)) {
#   Outcome <- my_out
#   
#   b1 <- obs.res_VegDiet_bin_con_ord$b[which(obs.res_VegDiet_bin_con_ord$Outcome == my_out)[1]]
#   se1 <- obs.res_VegDiet_bin_con_ord$se[which(obs.res_VegDiet_bin_con_ord$Outcome == my_out)[1]]
#   b2 <- obs.res_VegDiet_bin_con_ord$b[which(obs.res_VegDiet_bin_con_ord$Outcome == my_out)[2]]
#   se2 <- obs.res_VegDiet_bin_con_ord$se[which(obs.res_VegDiet_bin_con_ord$Outcome == my_out)[2]]
...
#     ""
#   
#   CCC_Z.test_bin_con_ord <- rbind(
#     CCC_Z.test_bin_con_ord,
#     c(
#       Outcome = Outcome,
#       `Z-value` = Z_value,
#       `p-value` = p_value,
...
# 
# write.xlsx(
#   as.data.frame(CCC_Z.test_bin_con_ord),
#   "results/BiB/IMP_CCC_Z.test_VegDiet_bin_con_ord.xlsx",
#   overwrite = T
# )
...
# Effect of SEP and health consciousness on vegetarianism and perinatal outcomes

## Prepare data
for (i in 1:dat_imp$m) {
  dat_WB[[i]]$edu_Mat_3cat <- as.numeric(dat_WB[[i]]$edu_Mat_3cat)
  dat_WB[[i]]$IMD_Fam_cat <- as.numeric(dat_WB[[i]]$IMD_Fam_cat)
  dat_WB[[i]]$conscious_Mat_ord <- as.numeric(dat_WB[[i]]$conscious_Mat_ord)
  dat_SA[[i]]$edu_Mat_3cat <- as.numeric(dat_SA[[i]]$edu_Mat_3cat)
  dat_SA[[i]]$IMD_Fam_cat <- as.numeric(dat_SA[[i]]$IMD_Fam_cat)
  dat_SA[[i]]$conscious_Mat_ord <- as.numeric(dat_SA[[i]]$conscious_Mat_ord)
}  # Recode confounding indicators as numeric to avoid model failure
...
for (myoutcome in c("VegDiet_bin", BiB_out_bin)) {
  ...
  ### 6 models - 2 SEP + 1 health consciousness indicators * 2 strata
  ...
  #### Model 3 - Health consciousness on vegetarianism and perinatal outcomes in White British
  mod_3 <- list()
  for (i in 1:dat_imp$m) {
    mod_3[[i]] <-
      glm(dat_WB[[i]][[myoutcome]] ~ dat_WB[[i]][["conscious_Mat_ord"]] + dat_WB[[i]][["age_Mat_con"]] + dat_WB[[i]][["edu_Mat_3cat"]]  + dat_WB[[i]][["IMD_Fam_cat"]] + dat_WB[[i]][["parity_Mat_bin"]],
          family = binomial)
  }
  mod_3_pooled <- pool(mod_3)
  y_3 <- IMP_extract_beta.SE(mod_3_pooled)
  
  #### Model 4 - Maternal education on vegetarianism and perinatal outcomes in South Asians
  mod_4 <- list()
  for (i in 1:dat_imp$m) {
    mod_4[[i]] <-
  ...
  #### Model 5 - IMD/household income on vegetarianism and perinatal outcomes in South Asians
  mod_5 <- list()
  for (i in 1:dat_imp$m) {
    mod_5[[i]] <-
  ...
  #### Model 6 - Health consciousness on vegetarianism and perinatal outcomes in South Asians
  mod_6 <- list()
  for (i in 1:dat_imp$m) {
    mod_6[[i]] <-
      glm(dat_SA[[i]][[myoutcome]] ~ dat_SA[[i]][["conscious_Mat_ord"]] + dat_SA[[i]][["age_Mat_con"]] + dat_SA[[i]][["edu_Mat_3cat"]]  + dat_SA[[i]][["IMD_Fam_cat"]] + dat_SA[[i]][["parity_Mat_bin"]],
          family = binomial)
  }
  ...
  confound_col <-
    as.data.frame(
      c(
        ...
        "Maternal adherence to healthy behaviours (higher vs. lower)",
        "Maternal education (higher vs. lower)",
        "IMD (more vs. less affluent)",
        "Maternal adherence to healthy behaviours (higher vs. lower)"
      )
    )
  colnames(confound_col) <- "Confounder"
  ethnic_col <-
    as.data.frame(
      c(
        "White British in BiB",
...
for (myoutcome in BiB_out_con) {
  ...
  for (i in 1:dat_imp$m) {
    mod_3[[i]] <-
      lm(dat_WB[[i]][[myoutcome]] ~ dat_WB[[i]][["conscious_Mat_ord"]] + dat_WB[[i]][["age_Mat_con"]] + dat_WB[[i]][["edu_Mat_3cat"]]  + dat_WB[[i]][["IMD_Fam_cat"]] + dat_WB[[i]][["parity_Mat_bin"]])
  }
  mod_3_pooled <- pool(mod_3)
  y_3 <- IMP_extract_beta.SE(mod_3_pooled)
  ...
  for (i in 1:dat_imp$m) {
    mod_6[[i]] <-
      lm(dat_SA[[i]][[myoutcome]] ~ dat_SA[[i]][["conscious_Mat_ord"]] + dat_SA[[i]][["age_Mat_con"]] + dat_SA[[i]][["edu_Mat_3cat"]]  + dat_SA[[i]][["IMD_Fam_cat"]] + dat_SA[[i]][["parity_Mat_bin"]])
  }
  mod_6_pooled <- pool(mod_6)
  y_6 <- IMP_extract_beta.SE(mod_6_pooled)
...
for (myoutcome in BiB_out_cat) {
  ...
  for (i in 1:dat_imp$m) {
    mod_3[[i]] <-
      clm(dat_WB[[i]][[myoutcome]] ~ dat_WB[[i]][["conscious_Mat_ord"]] + dat_WB[[i]][["age_Mat_con"]] + dat_WB[[i]][["edu_Mat_3cat"]]  + dat_WB[[i]][["IMD_Fam_cat"]] + dat_WB[[i]][["parity_Mat_bin"]])
  }
  mod_3_pooled <- pool(mod_3)
  ...
  for (i in 1:dat_imp$m) {
    mod_6[[i]] <-
      clm(dat_SA[[i]][[myoutcome]] ~ dat_SA[[i]][["conscious_Mat_ord"]] + dat_SA[[i]][["age_Mat_con"]] + dat_SA[[i]][["edu_Mat_3cat"]]  + dat_SA[[i]][["IMD_Fam_cat"]] + dat_SA[[i]][["parity_Mat_bin"]])
  }
  mod_6_pooled <- pool(mod_6)
...
colnames(obs.res_VegDiet_bin_con_ord)[colnames(obs.res_VegDiet_bin_con_ord) == "Confounder"] <- "Indicator of potential confounding"  # Change the column name to be shown in the forest plot
obs.res_VegDiet_bin_con_ord$`Indicator of potential confounding` <- factor(
  obs.res_VegDiet_bin_con_ord$`Indicator of potential confounding`,
  levels = c(
    "Maternal adherence to healthy behaviours (higher vs. lower)",
    "IMD (more vs. less affluent)",
    "Maternal education (higher vs. lower)"
...
dim(obs.res_VegDiet_bin_con_ord)  # 25 outcomes * 3 indicators * 2 ethnic groups = 150 obs.

################################################################################
#### Separate White British and South Asian results
obs.res_VegDiet_bin_con_ord_WB <- obs.res_VegDiet_bin_con_ord[obs.res_VegDiet_bin_con_ord$Ethnicity == "White British in BiB", ]
obs.res_VegDiet_bin_con_ord_SA <- obs.res_VegDiet_bin_con_ord[obs.res_VegDiet_bin_con_ord$Ethnicity == "South Asian in BiB", ]
dim(obs.res_VegDiet_bin_con_ord_WB)  # 75 obs.
dim(obs.res_VegDiet_bin_con_ord_SA)  # 75 obs.
################################################################################
#### Remove some results with extremely huge 95% CIs
...
dim(obs.res_VegDiet_bin_con_ord_WB)  # 75 -> 75 obs.
dim(obs.res_VegDiet_bin_con_ord_SA)  # 75 -> 75 obs.
################################################################################
...
dim(obs.res_VegDiet_bin_con_ord_WB)  # 75 -> 48 obs.
dim(obs.res_VegDiet_bin_con_ord_SA)  # 75 -> 48 obs.
################################################################################
...
obs.forest_VegDiet_bin_con_ord_WB <- ggforestplot::forestplot(
  ...
  pvalue = pval,
  psignif = 0.05,
  colour = `Indicator of potential confounding`,
  shape = `Indicator of potential confounding`,
  xlab = "LogOR or beta and 95% CI",
  title = "White British in BiB",
  logodds = F
) +
  ggplot2::scale_colour_manual(values = c("seagreen", "blueviolet", "violetred")) +
...
ggsave(
  obs.forest_VegDiet_bin_con_ord_WB,
  file = "results/BiB/IMP_CCC_SEP_obs.forest_VegDiet_bin_con_ord_WB.png",
  height = 8,
  width = 10
)
...
ggsave(
  obs.forest_VegDiet_bin_con_ord_SA,
  file = "results/BiB/IMP_CCC_SEP_obs.forest_VegDiet_bin_con_ord_SA.png",
  height = 8,
  width = 10
)
...
# # Combine cross-context comparison results for binary, continuous, and ordinal outcomes
# obs.forest_VegDiet_bin_ord <- obs.forest_VegDiet_bin_ord + ggplot2::theme(legend.position = "bottom")
# print(obs.forest_VegDiet_bin_ord)
# grob1 <- grid.grab()
# dev.off()
#
# obs.forest_VegDiet_con <- obs.forest_VegDiet_con + ggplot2::theme(legend.position = "bottom",
#                                                                   legend.justification = c(-1000, 0))
# print(obs.forest_VegDiet_con)
# grob2 <- grid.grab()
# dev.off()
#
...
# dev.off()
#
# title1 <- textGrob("Cross-context comparison", gp = gpar(fontsize = 16, fontface = "bold"))
# title2 <- textGrob(" ", gp = gpar(fontsize = 16, fontface = "bold"))
# title3 <- textGrob(
#   "Confounding structure of socioeconimic position (SEP)",
#   gp = gpar(fontsize = 16, fontface = "bold")
# )
# title4 <- textGrob(" ", gp = gpar(fontsize = 16, fontface = "bold"))
#
...
# grob3_with_title <- arrangeGrob(title3, grob3, ncol = 1, heights = c(0.05, 0.95))
# grob4_with_title <- arrangeGrob(title4, grob4, ncol = 1, heights = c(0.05, 0.95))
#
# png(
#   "results/BiB/Comb_IMP_CCC_SEP_obs.forest_VegDiet.png",
#   res = 300,
#   height = 3000,
#   width = 6000
# )
...
grid.arrange(grob1, grob2, ncol = 2, widths = c(1, 1))
dev.off()
