...
# This script is to perform stratification analyses (with imputed data) for vegetarian diets (in subgroups) in BiB.
...
for (i in 1:dat_imp$m) {
  mod_N_iron[[i]] <-
    glm(
      anaemia_preg_all ~ VegDiet_3cat + age_Mat_con + ethnic_Mat_bin + edu_Mat_3cat  + IMD_Fam_cat + parity_Mat_bin + BMI_Mat_PRE.p_con + smoking_Mat_EAR.p_bin + alcohol_Mat_EAR.p_bin + sex_Chi_bin + NON.iron.multivit.supp_Mat_EAR.p_bin,
...
write.xlsx(
  obs.res_VegDiet_bin,
  "results/BiB/IMP_STRAT.iron_obs.res_VegDiet.subgroup_bin.xlsx",
  overwrite = T
)
obs.res_VegDiet_bin <-
  read.xlsx("results/BiB/IMP_STRAT.iron_obs.res_VegDiet.subgroup_bin.xlsx")
...
ggsave(
  obs.forest_VegDiet_bin,
  file = "results/BiB/IMP_STRAT.iron_obs.forest_VegDiet.subgroup_bin.png",
  height = 2,
  width = 6
)
