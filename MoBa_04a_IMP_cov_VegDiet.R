...
# This script is to examine the association/effect of key covariates with/on vegetarianism (with imputed data) in MoBa.
...
for (i in 1:dat_imp$m) {
  ...
  dat[[i]]$pesco.V_bin[dat[[i]]$VegDiet_3cat == "Non-vegetarian"] <-
    0
  dat[[i]]$pesco.V_bin[dat[[i]]$VegDiet_3cat == "Pesco-vegetarian"] <-
    1
  var_lab(dat[[i]]$pesco.V_bin) <- "Pesco- vs. non-vegetarian"
  
  dat[[i]]$full.V_bin <- NA
  dat[[i]]$full.V_bin[dat[[i]]$VegDiet_3cat == "Non-vegetarian"] <-
    0
  dat[[i]]$full.V_bin[dat[[i]]$VegDiet_3cat == "Full vegetarian"] <-
    1
...
## Rename covariates
obs.res_VegDiet_bin$Covariate[obs.res_VegDiet_bin$Covariate == "age_Mat_con"] <-
  "Age\n(in years)"
obs.res_VegDiet_bin$Covariate[obs.res_VegDiet_bin$Covariate == "ethnic_Mat_bin"] <-
  "Ethnicity\n(Other vs. White)"
obs.res_VegDiet_bin$Covariate[obs.res_VegDiet_bin$Covariate == "edu_Mat_3cat"] <-
  "Education\n(higher vs. lower)"
obs.res_VegDiet_bin$Covariate[obs.res_VegDiet_bin$Covariate == "income_Fam_3cat"] <-
  "Household income\n(higher vs. lower)"
obs.res_VegDiet_bin$Covariate[obs.res_VegDiet_bin$Covariate == "parity_Mat_bin"] <-
  "Parity\n(≥1 vs. 0)"
obs.res_VegDiet_bin$Covariate[obs.res_VegDiet_bin$Covariate == "BMI_Mat_PRE.p_con"] <-
  "Pre-pregnancy BMI\n(in kg/m^2)"
obs.res_VegDiet_bin$Covariate[obs.res_VegDiet_bin$Covariate == "smoking_Mat_EAR.p_bin"] <-
  "Smoking\n(yes vs. no)"
obs.res_VegDiet_bin$Covariate[obs.res_VegDiet_bin$Covariate == "alcohol_Mat_EAR.p_bin"] <-
  "Alcohol drinking\n(yes vs. no)"
obs.res_VegDiet_bin$Covariate[obs.res_VegDiet_bin$Covariate == "any.supp_Mat_EAR.p_bin"] <-
  "Any supplement use\n(yes vs. no)"
obs.res_VegDiet_bin$Covariate[obs.res_VegDiet_bin$Covariate == "energy_Mat_DUR.p_con"] <-
  "Total energy intake\n(in kcal/day)"
obs.res_VegDiet_bin$Covariate[obs.res_VegDiet_bin$Covariate == "PDIm"] <-
  "Overall PDI"
obs.res_VegDiet_bin$Covariate[obs.res_VegDiet_bin$Covariate == "hPDIm"] <-
  "Healthful PDI"
obs.res_VegDiet_bin$Covariate[obs.res_VegDiet_bin$Covariate == "uPDIm"] <-
  "Unhealthful PDI"
obs.res_VegDiet_bin$Covariate[obs.res_VegDiet_bin$Covariate == "sex_Chi_bin"] <-
  "Offspring sex\n(female vs. male)"

obs.res_VegDiet_bin
dim(obs.res_VegDiet_bin)  # (14 covariates) * 2 vegetarian subgroups = 28 obs.

## Save results
write.xlsx(obs.res_VegDiet_bin,
           "results/IMP_cov_obs.res_VegDiet_bin.xlsx",
           overwrite = T)
