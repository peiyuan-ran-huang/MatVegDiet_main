...
# This script is to perform negative control outcome analysis (with imputed data) on breastfeeding for vegetarian diets (in subgroups) in Project Viva.
...
for (i in 1:dat_imp$m) {
  mod_1[[i]] <-
    clm(bf_dur_4c ~ VegDiet_3cat, data = dat[[i]])
}
...
for (i in 1:dat_imp$m) {
  mod_2[[i]] <-
    clm(
      bf_dur_4c ~ VegDiet_3cat + age_Mat_con + ethnic_Mat_bin + edu_Mat_3cat  + income_Fam_3cat + parity_Mat_bin + BMI_Mat_PRE.p_con + smoking_Mat_EAR.p_bin + alcohol_Mat_EAR.p_bin + sex_Chi_bin,
      data = dat[[i]]
    )
}
...
for (i in 1:dat_imp$m) {
  mod_3[[i]] <-
    clm(
      bf_dur_4c ~ VegDiet_3cat + age_Mat_con + ethnic_Mat_bin + edu_Mat_3cat  + income_Fam_3cat + parity_Mat_bin + BMI_Mat_PRE.p_con + smoking_Mat_EAR.p_bin + alcohol_Mat_EAR.p_bin + sex_Chi_bin + any.supp_Mat_EAR.p_bin + hPDIm,
      data = dat[[i]]
    )
}
...
obs.tbl_VegDiet_ord[, c(7, 11)] <- " "  # For convenience when making tables
obs.tbl_VegDiet_ord <- obs.tbl_VegDiet_ord[, c(1, 3, 2, 4:ncol(obs.tbl_VegDiet_ord))]  # For convenience when making tables
obs.tbl_VegDiet_ord
dim(obs.tbl_VegDiet_ord)  # 1 outcome for 3 categories in 3 models
write.xlsx(
  obs.tbl_VegDiet_ord,
  "results/Viva/IMP_BF_obs.tbl_VegDiet.subgroup_ord.xlsx",
  overwrite = T
)
obs.tbl_VegDiet_ord <-
  read.xlsx("results/Viva/IMP_BF_obs.tbl_VegDiet.subgroup_ord.xlsx")
...
write.xlsx(
  obs.res_VegDiet_ord,
  "results/Viva/IMP_BF_obs.res_VegDiet.subgroup_ord.xlsx",
  overwrite = T
)
obs.res_VegDiet_ord <-
  read.xlsx("results/Viva/IMP_BF_obs.res_VegDiet.subgroup_ord.xlsx")
...
### ...on pesco-vegetarianism (binary)
mod <- list()

for (i in 1:dat_imp$m) {
  mod[[i]] <- glm(
    VegDiet_3cat ~ edu_Mat_3cat + age_Mat_con + ethnic_Mat_bin,
    data = subset(
      dat[[i]],
      VegDiet_3cat %in% c("Non-vegetarian", "Pesco-vegetarian")
    ),
    family = binomial
  )
}
...
for (i in 1:dat_imp$m) {
  mod[[i]] <- glm(
    ...
    data = subset(
      dat[[i]],
      VegDiet_3cat %in% c("Non-vegetarian", "Full vegetarian")
    ),
    family = binomial
  )
}
...
for (i in 1:dat_imp$m) {
  mod[[i]] <- glm(
    VegDiet_3cat ~ income_Fam_3cat + age_Mat_con + ethnic_Mat_bin,
    data = subset(
      dat[[i]],
      VegDiet_3cat %in% c("Non-vegetarian", "Pesco-vegetarian")
    ),
...
obs.res_VegDiet_SEP
dim(obs.res_VegDiet_SEP)  # 2 exposures * 3 outcomes = 6 obs

write.xlsx(
  obs.res_VegDiet_SEP,
  "results/Viva/IMP_BF_obs.res_VegDiet.subgroup_SEP.xlsx",
  overwrite = T
)
obs.res_VegDiet_SEP <-
  read.xlsx("results/Viva/IMP_BF_obs.res_VegDiet.subgroup_SEP.xlsx")
...
obs.forest_VegDiet_SEP <- ggforestplot::forestplot(
  ...
  ggplot2::scale_colour_manual(values = c("palevioletred1", "green4", "mediumblue")) +
...
ggsave(
  obs.forest_VegDiet_SEP,
  file = "results/Viva/IMP_BF_obs.forest_VegDiet.subgroup_SEP.png",
  height = 3,
  width = 9
)
