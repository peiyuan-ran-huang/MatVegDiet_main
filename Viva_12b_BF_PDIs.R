...
# Last edited date: 04-Jun-2024
# This script is to perform negative control outcome analysis on breastfeeding for plant-based diet indices (PDIs) in Project Viva.
...
## Model 3: + dietary supplementation, total energy intake, hPDI - i.e., additionally adjusting for nutrition-related factors
################################################################################
...
extract_ord.log.res_3cat <- function(mymodel) {
...
x_1 <- extract_ord.log.res_3cat(mod_1)
...
x_2 <- extract_ord.log.res_3cat(mod_2)
...
try({
  mod_3 <-
    clm(
      bf_dur_4c ~ PDI_z + age_Mat_con + ethnic_Mat_bin + edu_Mat_3cat  + income_Fam_3cat + parity_Mat_bin + BMI_Mat_PRE.p_con + smoking_Mat_EAR.p_bin + alcohol_Mat_EAR.p_bin + sex_Chi_bin + any.supp_Mat_EAR.p_bin + energy_Mat_DUR.p_con,
      data = dat
    )
}, silent = T)
mod_3 <-
...
x_3 <- extract_ord.log.res_3cat(mod_3)
...
x_4 <- extract_ord.log.res_3cat(mod_4)
...
x_5 <- extract_ord.log.res_3cat(mod_5)
...
try({
  mod_6 <-
    clm(
      bf_dur_4c ~ hPDI_z + age_Mat_con + ethnic_Mat_bin + edu_Mat_3cat  + income_Fam_3cat + parity_Mat_bin + BMI_Mat_PRE.p_con + smoking_Mat_EAR.p_bin + alcohol_Mat_EAR.p_bin + sex_Chi_bin + any.supp_Mat_EAR.p_bin + energy_Mat_DUR.p_con,
      data = dat
    )
}, silent = T)
mod_6 <-
...
x_6 <- extract_ord.log.res_3cat(mod_6)
...
x_7 <- extract_ord.log.res_3cat(mod_7)
...
x_8 <- extract_ord.log.res_3cat(mod_8)
...
try({
  mod_9 <-
    clm(
      bf_dur_4c ~ uPDI_z + age_Mat_con + ethnic_Mat_bin + edu_Mat_3cat  + income_Fam_3cat + parity_Mat_bin + BMI_Mat_PRE.p_con + smoking_Mat_EAR.p_bin + alcohol_Mat_EAR.p_bin + sex_Chi_bin + any.supp_Mat_EAR.p_bin + energy_Mat_DUR.p_con,
      data = dat
    )
}, silent = T)
mod_9 <-
...
x_9 <- extract_ord.log.res_3cat(mod_9)
...
write.xlsx(
  obs.tbl_PDI.hPDI.uPDI_ord,
  "results/Viva/BF_obs.tbl_PDI.hPDI.uPDI_ord.xlsx",
  overwrite = T
)
obs.tbl_PDI.hPDI.uPDI_ord <-
  read.xlsx("results/Viva/BF_obs.tbl_PDI.hPDI.uPDI_ord.xlsx")
...
write.xlsx(obs.res_PDI_ord,
           "results/Viva/BF_obs.res_PDI_ord.xlsx",
           overwrite = T)
obs.res_PDI_ord <-
  read.xlsx("results/Viva/BF_obs.res_PDI_ord.xlsx")
...
write.xlsx(obs.res_hPDI_ord,
           "results/Viva/BF_obs.res_hPDI_ord.xlsx",
           overwrite = T)
obs.res_hPDI_ord <-
  read.xlsx("results/Viva/BF_obs.res_hPDI_ord.xlsx")
...
write.xlsx(obs.res_uPDI_ord,
           "results/Viva/BF_obs.res_uPDI_ord.xlsx",
           overwrite = T)
obs.res_uPDI_ord <-
  read.xlsx("results/Viva/BF_obs.res_uPDI_ord.xlsx")
...
### ...on PDI (continuous)
mod <- lm(PDI_z ~ edu_Mat_3cat, data = dat)
...
### ...on hPDI (continuous)
mod <- lm(hPDI_z ~ edu_Mat_3cat, data = dat)
...
### ...on uPDI (continuous)
mod <- lm(uPDI_z ~ edu_Mat_3cat, data = dat)
...
mod <- clm(bf_dur_4c ~ edu_Mat_3cat, data = dat, family = binomial)
...
### ...on PDI (continuous)
mod <- lm(PDI_z ~ income_Fam_3cat, data = dat)
...
### ...on hPDI (continuous)
mod <- lm(hPDI_z ~ income_Fam_3cat, data = dat)
...
### ...on uPDI (continuous)
mod <- lm(uPDI_z ~ income_Fam_3cat, data = dat)
...
### ...on breastfeeding duration (ordinal)
mod <- clm(bf_dur_4c ~ income_Fam_3cat,
           data = dat,
           family = binomial)
...
write.xlsx(
  obs.res_PDI.hPDI.uPDI_SEP,
  "results/Viva/BF_obs.res_PDI.hPDI.uPDI_SEP.xlsx",
  overwrite = T
)
obs.res_PDI.hPDI.uPDI_SEP <-
  read.xlsx("results/Viva/BF_obs.res_PDI.hPDI.uPDI_SEP.xlsx")
...
ggsave(
  "results/Viva/BF_obs.forest_PDI.hPDI.uPDI_SEP.png",
