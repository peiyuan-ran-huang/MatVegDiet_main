...
# This script is to perform main association analysis (with imputed data) for vegetarian diets in ALSPAC.
...
for (myoutcome in ALSPAC_out_bin) {
  ...
  for (i in 1:dat_imp$m) {
    mod_3[[i]] <-
      glm(
        dat[[i]][[myoutcome]] ~ dat[[i]][["VegDiet_bin"]] + dat[[i]][["age_Mat_con"]] + dat[[i]][["ethnic_Mat_bin"]] + dat[[i]][["edu_Mat_3cat"]]  + dat[[i]][["IMD_Fam_cat"]] + dat[[i]][["parity_Mat_bin"]] + dat[[i]][["BMI_Mat_PRE.p_con"]] + dat[[i]][["smoking_Mat_EAR.p_bin"]] + dat[[i]][["alcohol_Mat_EAR.p_bin"]] + dat[[i]][["sex_Chi_bin"]] + dat[[i]][["any.supp_Mat_EAR.p_bin"]] + dat[[i]][["energy_Mat_DUR.p_con"]] + dat[[i]][["hPDIm"]],
        family = binomial
      )
  }
...
write.xlsx(
  obs.tbl_VegDiet_bin,
  "results/ALSPAC/IMP_MAIN_obs.tbl_VegDiet_bin.xlsx",
  overwrite = T
)
obs.tbl_VegDiet_bin <-
  read.xlsx("results/ALSPAC/IMP_MAIN_obs.tbl_VegDiet_bin.xlsx")
...
write.xlsx(
  obs.res_VegDiet_bin,
  "results/ALSPAC/IMP_MAIN_obs.res_VegDiet_bin.xlsx",
  overwrite = T
)
obs.res_VegDiet_bin <-
  read.xlsx("results/ALSPAC/IMP_MAIN_obs.res_VegDiet_bin.xlsx")
...
ggsave(
  obs.forest_VegDiet_bin,
  file = "results/ALSPAC/IMP_MAIN_obs.forest_VegDiet_bin.png",
  height = 5,
  width = 8
)
...
write.xlsx(
  obs.tbl_VegDiet_con,
  "results/ALSPAC/IMP_MAIN_obs.tbl_VegDiet_con.xlsx",
  overwrite = T
)
obs.tbl_VegDiet_con <-
  read.xlsx("results/ALSPAC/IMP_MAIN_obs.tbl_VegDiet_con.xlsx")
...
write.xlsx(
  obs.res_VegDiet_con,
  "results/ALSPAC/IMP_MAIN_obs.res_VegDiet_con.xlsx",
  overwrite = T
)
obs.res_VegDiet_con <-
  read.xlsx("results/ALSPAC/IMP_MAIN_obs.res_VegDiet_con.xlsx")
...
ggsave(
  obs.forest_VegDiet_con,
  file = "results/ALSPAC/IMP_MAIN_obs.forest_VegDiet_con.png",
  height = 2,
  width = 6
)
