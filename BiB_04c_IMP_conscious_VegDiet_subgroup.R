...
# This script is to examine the effect of health behaviours/consciousness on both vegetarianism (in subgroups) and perinatal outcomes (with imputed data) in BiB.
...
for (i in 1:dat_imp$m) {
  ...
  var_lab(dat[[i]]$conscious_Mat_ord) <- "Maternal composite lifestyle index (0–4)"
}
...
for (myoutcome in c("VegDiet_bin",
  ...
  for (i in 1:dat_imp$m) {
    mod_1[[i]] <-
      glm(dat[[i]][[myoutcome]] ~ dat[[i]][["conscious_Mat_ord"]] + dat[[i]][["age_Mat_con"]] + dat[[i]][["ethnic_Mat_bin"]] + dat[[i]][["edu_Mat_3cat"]] + dat[[i]][["IMD_Fam_cat"]] + dat[[i]][["parity_Mat_bin"]], family = binomial)
  }
...
for (myoutcome in BiB_out_con) {
  ...
  for (i in 1:dat_imp$m) {
    mod_1[[i]] <-
      lm(dat[[i]][[myoutcome]] ~ dat[[i]][["conscious_Mat_ord"]] + dat[[i]][["age_Mat_con"]] + dat[[i]][["ethnic_Mat_bin"]] + dat[[i]][["edu_Mat_3cat"]] + dat[[i]][["IMD_Fam_cat"]] + dat[[i]][["parity_Mat_bin"]])
  }
  mod_1_pooled <- pool(mod_1)
  y_1 <- IMP_extract_beta.SE(mod_1_pooled)
...
for (myoutcome in BiB_out_cat) {
  ...
  for (i in 1:dat_imp$m) {
    mod_1[[i]] <-
      clm(dat[[i]][[myoutcome]] ~ dat[[i]][["conscious_Mat_ord"]] + dat[[i]][["age_Mat_con"]] + dat[[i]][["ethnic_Mat_bin"]] + dat[[i]][["edu_Mat_3cat"]] + dat[[i]][["IMD_Fam_cat"]] + dat[[i]][["parity_Mat_bin"]])
  }
  mod_1_pooled <- pool(mod_1)
...
obs.res_VegDiet_bin
dim(obs.res_VegDiet_bin)  # (2 + 1 exposures + 19 outcomes) * 1 model = 22 obs.
...
obs.res_VegDiet_bin_con_ord <- rbind(obs.res_VegDiet_bin_con, obs.res_VegDiet_ord)

write.xlsx(
  obs.res_VegDiet_bin_con_ord,
  "results/BiB/IMP_conscious_obs.res_VegDiet.subgroup_bin_con_ord.xlsx",
  overwrite = T
)
obs.res_VegDiet_bin_con_ord <-
  read.xlsx("results/BiB/IMP_conscious_obs.res_VegDiet.subgroup_bin_con_ord.xlsx")
...
### Prepare data
obs.res_VegDiet_bin_con_ord$Group <- "Perinatal outcome (binary/continuous)"
...
colnames(obs.res_VegDiet_bin_con_ord)[colnames(obs.res_VegDiet_bin_con_ord) == "HC"] <- "Indicator"  # Change the column name to be shown in the forest plot
...
dim(obs.res_VegDiet_bin_con_ord)  # 27 outcomes * 1 model = 27 obs.
...
obs.forest_VegDiet_bin_con_ord <- ggforestplot::forestplot(
  ...
  title = "Effect of maternal health behaviours/consciousness in BiB",
  logodds = F
) +
...
ggsave(
  obs.forest_VegDiet_bin_con_ord,
  file = "results/BiB/IMP_conscious_obs.forest_VegDiet.subgroup_bin_con_ord.png",
  height = 7,
  width = 10
)
