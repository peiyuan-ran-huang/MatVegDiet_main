...
# Last edited date: 13-Oct-2024
# This script is to examine the effect of socioeconomic position (SEP) on both vegetarianism (in subgroups) and perinatal outcomes (with imputed data) in ALSPAC.
...
for (myoutcome in c("VegDiet_bin",
  ...
  for (i in 1:dat_imp$m) {
    mod_2[[i]] <-
      glm(dat[[i]][[myoutcome]] ~ dat[[i]][["IMD_Fam_cat"]] + dat[[i]][["age_Mat_con"]] + dat[[i]][["ethnic_Mat_bin"]], family = binomial)
  }
...
for (myoutcome in ALSPAC_out_con) {
  ...
  for (i in 1:dat_imp$m) {
    mod_2[[i]] <-
      lm(dat[[i]][[myoutcome]] ~ dat[[i]][["IMD_Fam_cat"]] + dat[[i]][["age_Mat_con"]] + dat[[i]][["ethnic_Mat_bin"]])
  }
  mod_2_pooled <- pool(mod_2)
  y_2 <- IMP_extract_beta.SE_ord(mod_2_pooled)
...
obs.res_VegDiet_bin
dim(obs.res_VegDiet_bin)  # (2 + 1 exposures + 21 outcomes) * 2 models = 48 obs.

obs.res_VegDiet_con <- as.data.frame(obs.res_VegDiet_con)
obs.res_VegDiet_con
dim(obs.res_VegDiet_con)  # 4 outcomes * 2 models = 8 obs.

obs.res_VegDiet_bin_con <- rbind(obs.res_VegDiet_bin, obs.res_VegDiet_con)

write.xlsx(
  obs.res_VegDiet_bin_con,
  "results/ALSPAC/IMP_SEP_obs.res_VegDiet.subgroup_bin_con.xlsx",
  overwrite = T
)
obs.res_VegDiet_bin_con <-
  read.xlsx("results/ALSPAC/IMP_SEP_obs.res_VegDiet.subgroup_bin_con.xlsx")
...
dim(obs.res_VegDiet_bin_con)  # 28 outcomes * 2 models = 56 obs.
...
obs.forest_VegDiet_bin_con <- ggforestplot::forestplot(
  ...
  title = "Effect of socioeconomic position (SEP) in ALSPAC",
  logodds = F
) +
...
ggsave(
  obs.forest_VegDiet_bin_con,
  file = "results/ALSPAC/IMP_SEP_obs.forest_VegDiet.subgroup_bin_con.png",
  height = 10,
  width = 10
)
