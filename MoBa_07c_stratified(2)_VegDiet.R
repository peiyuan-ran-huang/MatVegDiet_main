...
x_iron_3 <-
  as.data.frame(extract_log.res_bin(mod_iron_3))
y_iron_3 <- extract_beta.SE_bin(mod_iron_3)

y_iron_3$Outcome <- "Maternal anaemia during pregnancy"
y_iron_3$`Iron supplement use` <- "Started before pregnancy"

for (i in 1:nrow(y_iron_3)) {
  y_iron_3$N_exp[i] <-
    x_iron_3$N[x_iron_3$Exposure == y_iron_3$Exposure[i]]
  y_iron_3$N_ref[i] <-
    x_iron_3$N[x_iron_3$Exposure == "Non-vegetarian (ref)"]
}
...
write.xlsx(
  obs.res_VegDiet_bin,
  "results/STRAT.iron.dur_obs.res_VegDiet_bin.xlsx",
  overwrite = T
)
...
ggsave(
  obs.forest_VegDiet_bin,
  file = "results/STRAT.iron.dur_obs.forest_VegDiet_bin.png",
  height = 3,
  width = 6
)
