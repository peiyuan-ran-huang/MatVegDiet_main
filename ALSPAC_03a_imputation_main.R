...
# This script is to perform multiple imputation by chained equations (MICE) for main association analysis in ALSPAC.
...
substantive_var <-
  read.xlsx("data/ALSPAC/select_var.xlsx", sheet = "Substantive")
...
auxiliary_var <-
  read.xlsx("data/ALSPAC/select_var.xlsx", sheet = "Auxiliary")
...
other_N_imp <-
  c("iron.supp_Mat_EAR.p_bin",
    "VegDiet_bin",
    "VegDiet_bin_FFQ_self")  # Other variables NOT to be imputed (e.g., used for stratification or as alternative exposure)
################################################################################
...
sum(is.na(dat$PDIm))  # 104
sum(is.na(dat$hPDIm))  # 104
sum(is.na(dat$uPDIm))  # 104
...
(num_incomplete_cases / num_total_cases) * 100  # Percentage of incomplete cases: 24.12086%
...
## Save imputed data
save(dat_imp, file = "data/ALSPAC/dat_exp_cov_out_pat_IMP_main.RData")
...
tab_ori_imp %>% as_flex_table() %>% flextable::save_as_docx(., path =
                                                              "results/ALSPAC/00a-maternal_covariates_OBS.vs.IMP.docx")
