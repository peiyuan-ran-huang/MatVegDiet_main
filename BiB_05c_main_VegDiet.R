...
# Last edited date: 01-Dec-2024
# This script is to perform main association analysis for vegetarian diets in BiB.
...
## Model 3: + dietary supplementation - i.e., additionally adjusting for nutrition-related factors
################################################################################
...
for (myoutcome in BiB_out_bin) {
  ...
  mod_3 <-
    glm(dat[[myoutcome]] ~ dat[["VegDiet_bin"]] + dat[["age_Mat_con"]] + dat[["ethnic_Mat_bin"]] + dat[["edu_Mat_3cat"]]  + dat[["IMD_Fam_cat"]] + dat[["parity_Mat_bin"]] + dat[["BMI_Mat_PRE.p_con"]] + dat[["smoking_Mat_EAR.p_bin"]] + dat[["alcohol_Mat_EAR.p_bin"]] + dat[["sex_Chi_bin"]] + dat[["any.supp_Mat_EAR.p_bin"]],
        family = binomial)
...
write.xlsx(obs.tbl_VegDiet_bin,
           "results/BiB/MAIN_obs.tbl_VegDiet_bin.xlsx",
           overwrite = T)
obs.tbl_VegDiet_bin <-
  read.xlsx("results/BiB/MAIN_obs.tbl_VegDiet_bin.xlsx")
...
write.xlsx(obs.res_VegDiet_bin,
           "results/BiB/MAIN_obs.res_VegDiet_bin.xlsx",
           overwrite = T)
obs.res_VegDiet_bin <-
  read.xlsx("results/BiB/MAIN_obs.res_VegDiet_bin.xlsx")
...
dim(obs.res_VegDiet_bin)  # 19 outcomes * 1 category * 1 model = 19 obs
...
head(obs.res_VegDiet_bin)
dim(obs.res_VegDiet_bin)  # 19 -> 14 obs
################################################################################
...
ggsave(
  obs.forest_VegDiet_bin,
  file = "results/BiB/MAIN_obs.forest_VegDiet_bin.png",
  height = 5,
  width = 8
)
...
for (myoutcome in BiB_out_con) {
  ...
  #### Model 3 - Additionally adjusting for nutrition-related factors
  mod_3 <-
    lm(dat[[myoutcome]] ~ dat[["VegDiet_bin"]] + dat[["age_Mat_con"]] + dat[["ethnic_Mat_bin"]] + dat[["edu_Mat_3cat"]]  + dat[["IMD_Fam_cat"]] + dat[["parity_Mat_bin"]] + dat[["BMI_Mat_PRE.p_con"]] + dat[["smoking_Mat_EAR.p_bin"]] + dat[["alcohol_Mat_EAR.p_bin"]] + dat[["sex_Chi_bin"]] + dat[["any.supp_Mat_EAR.p_bin"]])
...
write.xlsx(obs.tbl_VegDiet_con,
           "results/BiB/MAIN_obs.tbl_VegDiet_con.xlsx",
           overwrite = T)
obs.tbl_VegDiet_con <-
  read.xlsx("results/BiB/MAIN_obs.tbl_VegDiet_con.xlsx")
...
write.xlsx(obs.res_VegDiet_con,
           "results/BiB/MAIN_obs.res_VegDiet_con.xlsx",
           overwrite = T)
obs.res_VegDiet_con <-
  read.xlsx("results/BiB/MAIN_obs.res_VegDiet_con.xlsx")
...
obs.forest_VegDiet_con <- ggforestplot::forestplot(
  ...
  title = "Continuous outcomes in BiB",
  logodds = F
) +
  ggplot2::scale_colour_manual(values = c("darkcyan")) +
  ggplot2::scale_shape_manual(values = c(21))
...
ggsave(
  obs.forest_VegDiet_con,
  file = "results/BiB/MAIN_obs.forest_VegDiet_con.png",
  height = 2,
  width = 6
)
