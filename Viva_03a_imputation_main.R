...
# This script is to perform multiple imputation by chained equations (MICE) in Project Viva.
...
substantive_var <-
  read.xlsx("data/Viva/select_var.xlsx", sheet = "Substantive")
...
auxiliary_var <-
  read.xlsx("data/Viva/select_var.xlsx", sheet = "Auxiliary")
...
sum(is.na(dat$PDIm))  # 0
sum(is.na(dat$hPDIm))  # 0
sum(is.na(dat$uPDIm))  # 0
...
# [9] "any.supp_Mat_EAR.p_bin" "energy_Mat_DUR.p_con"   "hPDIm"                  "sex_Chi_bin"
...
cat("Highly correlated variables (cutoff = 0.9):",
    high_corr_vars,
    "\n")  # energy_Mat_DUR.p_con (OK as energy_Mat_EAR.p_con will not included in imputation)
...
## Save imputed data
save(dat_imp, file = "data/Viva/dat_exp_cov_out_IMP.RData")
...
tab_ori_imp %>% as_flex_table() %>% flextable::save_as_docx(., path =
                                                              "results/Viva/00-maternal_covariates_OBS.vs.IMP.docx")
