...
# This script is to perform sensitivity analysis (with imputed data) for vegetarian diets in MoBa.
## Sensitivity Analysis 2 - FFQ-based + self-defined vegetarianism (3-categories)
...
################################################################################
# Sensitivity Analysis 2 - Diet-based + self-defined vegetarianism (3-categories)
################################################################################

# Subset to those with non-missing diet-based + self-defined vegetarianism variable
for (i in 1:dat_imp$m) {
  dat[[i]] <- subset(dat[[i]], !is.na(dat[[i]]$VegDiet_3cat_FFQ_self))
}
...
for (myoutcome in MoBa_out_bin) {
  ...
  for (i in 1:dat_imp$m) {
    mod_1[[i]] <-
      glm(dat[[i]][[myoutcome]] ~ dat[[i]][["VegDiet_3cat_FFQ_self"]], family = binomial)
  }
  ...
  for (i in 1:dat_imp$m) {
    mod_2[[i]] <-
      glm(dat[[i]][[myoutcome]] ~ dat[[i]][["VegDiet_3cat_FFQ_self"]] + dat[[i]][["age_Mat_con"]] + dat[[i]][["ethnic_Mat_bin"]] + dat[[i]][["edu_Mat_3cat"]]  + dat[[i]][["income_Fam_3cat"]] + dat[[i]][["parity_Mat_bin"]] + dat[[i]][["BMI_Mat_PRE.p_con"]] + dat[[i]][["smoking_Mat_EAR.p_bin"]] + dat[[i]][["alcohol_Mat_EAR.p_bin"]] + dat[[i]][["sex_Chi_bin"]],
          family = binomial)
  }
  ...
  for (i in 1:dat_imp$m) {
    mod_3[[i]] <-
      glm(
        dat[[i]][[myoutcome]] ~ dat[[i]][["VegDiet_3cat_FFQ_self"]] + dat[[i]][["age_Mat_con"]] + dat[[i]][["ethnic_Mat_bin"]] + dat[[i]][["edu_Mat_3cat"]]  + dat[[i]][["income_Fam_3cat"]] + dat[[i]][["parity_Mat_bin"]] + dat[[i]][["BMI_Mat_PRE.p_con"]] + dat[[i]][["smoking_Mat_EAR.p_bin"]] + dat[[i]][["alcohol_Mat_EAR.p_bin"]] + dat[[i]][["sex_Chi_bin"]] + dat[[i]][["any.supp_Mat_EAR.p_bin"]] + dat[[i]][["energy_Mat_DUR.p_con"]] + dat[[i]][["hPDIm"]],
        family = binomial
      )
  }
  ...
  N_x <- as.data.frame(c(
    paste0(nrow(dat_1[dat_1$VegDiet_3cat_FFQ_self == "Non-vegetarian" &
                        !is.na(dat_1[[myoutcome]]) &
                        dat_1[[myoutcome]] == 1, ]), " / ", nrow(dat_1[dat_1$VegDiet_3cat_FFQ_self == "Non-vegetarian" &
                                                                         !is.na(dat_1[[myoutcome]]), ])),
    paste0(nrow(dat_1[dat_1$VegDiet_3cat_FFQ_self == "Pesco-vegetarian" &
                        !is.na(dat_1[[myoutcome]]) &
                        dat_1[[myoutcome]] == 1, ]), " / ", nrow(dat_1[dat_1$VegDiet_3cat_FFQ_self == "Pesco-vegetarian" &
                                                                         !is.na(dat_1[[myoutcome]]), ])),
    paste0(nrow(dat_1[dat_1$VegDiet_3cat_FFQ_self == "Full vegetarian" &
                        !is.na(dat_1[[myoutcome]]) &
                        dat_1[[myoutcome]] == 1, ]), " / ", nrow(dat_1[dat_1$VegDiet_3cat_FFQ_self == "Full vegetarian" &
                                                                         !is.na(dat_1[[myoutcome]]), ]))
  ))
  colnames(N_x) <- "N"
...
write.xlsx(
  obs.tbl_VegDiet_bin,
  "results/IMP_SENS.FFQ.self_obs.tbl_VegDiet.subgroup_bin.xlsx",
  overwrite = T
)
obs.tbl_VegDiet_bin <-
  read.xlsx("results/IMP_SENS.FFQ.self_obs.tbl_VegDiet.subgroup_bin.xlsx")
...
write.xlsx(
  obs.res_VegDiet_bin,
  "results/IMP_SENS.FFQ.self_obs.res_VegDiet.subgroup_bin.xlsx",
  overwrite = T
)
obs.res_VegDiet_bin <-
  read.xlsx("results/IMP_SENS.FFQ.self_obs.res_VegDiet.subgroup_bin.xlsx")
...
ggsave(
  obs.forest_VegDiet_bin,
  file = "results/IMP_SENS.FFQ.self_obs.forest_VegDiet.subgroup_bin.png",
  height = 8,
  width = 8
)
...
for (myoutcome in MoBa_out_con) {
  ...
  for (i in 1:dat_imp$m) {
    mod_1[[i]] <-
      lm(dat[[i]][[myoutcome]] ~ dat[[i]][["VegDiet_3cat_FFQ_self"]])
  }
  mod_1_pooled <- pool(mod_1)
  ...
  for (i in 1:dat_imp$m) {
    mod_2[[i]] <-
      lm(dat[[i]][[myoutcome]] ~ dat[[i]][["VegDiet_3cat_FFQ_self"]] + dat[[i]][["age_Mat_con"]] + dat[[i]][["ethnic_Mat_bin"]] + dat[[i]][["edu_Mat_3cat"]] + dat[[i]][["income_Fam_3cat"]] + dat[[i]][["parity_Mat_bin"]] + dat[[i]][["BMI_Mat_PRE.p_con"]] + dat[[i]][["smoking_Mat_EAR.p_bin"]] + dat[[i]][["alcohol_Mat_EAR.p_bin"]] + dat[[i]][["sex_Chi_bin"]])
  }
  mod_2_pooled <- pool(mod_2)
  ...
  N_x <-
    as.data.frame(c(nrow(dat_1[dat_1$VegDiet_3cat_FFQ_self == "Non-vegetarian" &
                                 !is.na(dat_1[[myoutcome]]), ]), nrow(dat_1[dat_1$VegDiet_3cat_FFQ_self == "Pesco-vegetarian" &
                                                                              !is.na(dat_1[[myoutcome]]), ]), nrow(dat_1[dat_1$VegDiet_3cat_FFQ_self == "Full vegetarian" &
                                                                                                                           !is.na(dat_1[[myoutcome]]), ])))
  colnames(N_x) <- "N"
...
write.xlsx(
  obs.tbl_VegDiet_con,
  "results/IMP_SENS.FFQ.self_obs.tbl_VegDiet.subgroup_con.xlsx",
  overwrite = T
)
obs.tbl_VegDiet_con <-
  read.xlsx("results/IMP_SENS.FFQ.self_obs.tbl_VegDiet.subgroup_con.xlsx")
...
write.xlsx(
  obs.res_VegDiet_con,
  "results/IMP_SENS.FFQ.self_obs.res_VegDiet.subgroup_con.xlsx",
  overwrite = T
)
obs.res_VegDiet_con <-
  read.xlsx("results/IMP_SENS.FFQ.self_obs.res_VegDiet.subgroup_con.xlsx")
...
ggsave(
  obs.forest_VegDiet_con,
  file = "results/IMP_SENS.FFQ.self_obs.forest_VegDiet.subgroup_con.png",
  height = 3,
  width = 6
)
...
# Vegetarian subgroups and breastfeeding duration (4 categories) - Ordinal logistic regression
...
for (i in 1:dat_imp$m) {
  mod_1[[i]] <-
    clm(bf_dur_4c ~ VegDiet_3cat_FFQ_self, data = dat[[i]])
}
mod_1_pooled <- pool(mod_1)
...
for (i in 1:dat_imp$m) {
  mod_2[[i]] <-
    clm(
      bf_dur_4c ~ VegDiet_3cat_FFQ_self + age_Mat_con + ethnic_Mat_bin + edu_Mat_3cat + income_Fam_3cat + parity_Mat_bin + BMI_Mat_PRE.p_con + smoking_Mat_EAR.p_bin + alcohol_Mat_EAR.p_bin + sex_Chi_bin,
      data = dat[[i]]
    )
}
mod_2_pooled <- pool(mod_2)
...
for (i in 1:dat_imp$m) {
  mod_3[[i]] <-
    clm(
      bf_dur_4c ~ VegDiet_3cat_FFQ_self + age_Mat_con + ethnic_Mat_bin + edu_Mat_3cat + income_Fam_3cat + parity_Mat_bin + BMI_Mat_PRE.p_con + smoking_Mat_EAR.p_bin + alcohol_Mat_EAR.p_bin + sex_Chi_bin + any.supp_Mat_EAR.p_bin + hPDIm,
      data = dat[[i]]
    )
}
mod_3_pooled <- pool(mod_3)
...
N_x <-
  as.data.frame(c(nrow(dat_1[dat_1$VegDiet_3cat_FFQ_self == "Non-vegetarian" &
                               !is.na(dat_1$bf_dur_4c), ]), nrow(dat_1[dat_1$VegDiet_3cat_FFQ_self == "Pesco-vegetarian" &
                                                                         !is.na(dat_1$bf_dur_4c), ]), nrow(dat_1[dat_1$VegDiet_3cat_FFQ_self == "Full vegetarian" &
...
write.xlsx(
  obs.tbl_VegDiet_ord,
  "results/IMP_SENS.FFQ.self_obs.tbl_VegDiet.subgroup_ord.xlsx",
  overwrite = T
)
obs.tbl_VegDiet_ord <-
  read.xlsx("results/IMP_SENS.FFQ.self_obs.tbl_VegDiet.subgroup_ord.xlsx")
...
obs.res_VegDiet_ord
dim(obs.res_VegDiet_ord)  # 1 outcome * 2 categories * 3 models = 6 obs
write.xlsx(
  obs.res_VegDiet_ord,
  "results/IMP_SENS.FFQ.self_obs.res_VegDiet.subgroup_ord.xlsx",
  overwrite = T
)
obs.res_VegDiet_ord <-
  read.xlsx("results/IMP_SENS.FFQ.self_obs.res_VegDiet.subgroup_ord.xlsx")
