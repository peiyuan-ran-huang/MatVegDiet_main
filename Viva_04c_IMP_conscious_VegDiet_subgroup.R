...
# This script is to examine the effect of health behaviours/consciousness on both vegetarianism (in subgroups) and perinatal outcomes (with imputed data) in Project Viva.
...
obs.res_VegDiet_bin
dim(obs.res_VegDiet_bin)  # (2 + 1 exposures + 17 outcomes) * 1 model = 20 obs.

obs.res_VegDiet_con <- as.data.frame(obs.res_VegDiet_con)
obs.res_VegDiet_con
dim(obs.res_VegDiet_con)  # 2 outcomes * 1 model = 2 obs.
...
write.xlsx(
  ...
  "results/Viva/IMP_conscious_obs.res_VegDiet.subgroup_bin_con_ord.xlsx",
  overwrite = T
)
obs.res_VegDiet_bin_con_ord <-
  read.xlsx("results/Viva/IMP_conscious_obs.res_VegDiet.subgroup_bin_con_ord.xlsx")
...
dim(obs.res_VegDiet_bin_con_ord)  # 23 outcomes * 1 model = 23 obs.
...
obs.forest_VegDiet_bin_con_ord <- ggforestplot::forestplot(
  ...
  title = "Effect of maternal health behaviours/consciousness in Project Viva",
  logodds = F
) +
...
ggsave(
  obs.forest_VegDiet_bin_con_ord,
  file = "results/Viva/IMP_conscious_obs.forest_VegDiet.subgroup_bin_con_ord.png",
  height = 7,
  width = 10
)
