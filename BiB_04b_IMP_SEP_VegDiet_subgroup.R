...
# This script is to examine socioeconomic position (SEP) on both vegetarianism (in subgroups) and perinatal outcomes (with imputed data) in BiB.
...
obs.res_VegDiet_bin
dim(obs.res_VegDiet_bin)  # (2 + 1 exposures + 19 outcomes) * 2 models = 44 obs.
...
write.xlsx(
  ...
  "results/BiB/IMP_SEP_obs.res_VegDiet.subgroup_bin_con.xlsx",
  overwrite = T
)
obs.res_VegDiet_bin_con <-
  read.xlsx("results/BiB/IMP_SEP_obs.res_VegDiet.subgroup_bin_con.xlsx")
...
dim(obs.res_VegDiet_bin_con)  # 26 outcomes * 2 models = 52 obs.
...
obs.forest_VegDiet_bin_con <- ggforestplot::forestplot(
  ...
  title = "Effect of socioeconomic position (SEP) in BiB",
  logodds = F
) +
...
ggsave(
  obs.forest_VegDiet_bin_con,
  file = "results/BiB/IMP_SEP_obs.forest_VegDiet.subgroup_bin_con.png",
  height = 10,
  width = 10
)
