...
# This script is to perform multiple imputation by chained equations (MICE) for plant-based diet indices (PDIs) in MoBa.
...
substantive_var <-
  read.xlsx("select_var.xlsx", sheet = "Substantive_PDIs")
...
auxiliary_var <-
  read.xlsx("select_var.xlsx", sheet = "Auxiliary_PDIs")
...
(num_incomplete_cases / num_total_cases) * 100  # Percentage of incomplete cases: 27.32577%
...
## Save imputed data
save(dat_imp, file = "dat_exp_cov_out_pat_IMP_PDIs.RData")
...
tab_ori_imp %>% as_flex_table() %>% flextable::save_as_docx(., path =
                                                              "results/00c-maternal_covariates_OBS.vs.IMP_PDIs.docx")
