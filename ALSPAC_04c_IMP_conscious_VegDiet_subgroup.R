...
# This script is to examine the effect of health behaviours/consciousness on both vegetarianism (in subgroups) and perinatal outcomes (with imputed data) in ALSPAC.
...
for (myoutcome in ALSPAC_out_cat) {
  ### Outcome name
  outcome <- var_lab(dat_1[myoutcome])
...
write.xlsx(
  ...
  "results/ALSPAC/IMP_conscious_obs.res_VegDiet.subgroup_bin_con_ord.xlsx",
  overwrite = T
)
obs.res_VegDiet_bin_con_ord <-
  read.xlsx("results/ALSPAC/IMP_conscious_obs.res_VegDiet.subgroup_bin_con_ord.xlsx")
...
obs.forest_VegDiet_bin_con_ord <- ggforestplot::forestplot(
  ...
  title = "Effect of maternal health behaviours/consciousness in ALSPAC",
  logodds = F
) +
...
ggsave(
  obs.forest_VegDiet_bin_con_ord,
  file = "results/ALSPAC/IMP_conscious_obs.forest_VegDiet.subgroup_bin_con_ord.png",
  height = 7,
  width = 10
)
