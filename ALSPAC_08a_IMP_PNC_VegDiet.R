...
for (myoutcome in ALSPAC_out_bin) {
  ...
  for (i in 1:dat_imp$m) {
    mod_3[[i]] <-
      glm(dat[[i]][[myoutcome]] ~ dat[[i]][["VegDiet_bin_Pat"]] + dat[[i]][["age_Pat_con"]] + dat[[i]][["ethnic_Pat_bin"]] + dat[[i]][["edu_Pat_3cat"]]  + dat[[i]][["IMD_Fam_cat"]] + dat[[i]][["parity_Pat_bin"]] + dat[[i]][["BMI_Pat_EAR.p_con"]] + dat[[i]][["smoking_Pat_EAR.p_bin"]] + dat[[i]][["alcohol_Pat_EAR.p_bin"]] + dat[[i]][["sex_Chi_bin"]],
          family = binomial)
  }
  ...
  N_x_Mat <- as.data.frame(c(paste0(nrow(dat_1[dat_1$VegDiet_bin == 0 &
  ...
  N_x_Pat <- as.data.frame(c(paste0(nrow(dat_1[dat_1$VegDiet_bin_Pat == 0 &
                                                 !is.na(dat_1[[myoutcome]]) &
...
write.xlsx(
  obs.tbl_VegDiet_bin,
  "results/ALSPAC/IMP_PNC_obs.tbl_VegDiet_bin.xlsx",
  overwrite = T
)
obs.tbl_VegDiet_bin <-
  read.xlsx("results/ALSPAC/IMP_PNC_obs.tbl_VegDiet_bin.xlsx")
...
write.xlsx(
  obs.res_VegDiet_bin,
  "results/ALSPAC/IMP_PNC_obs.res_VegDiet_bin.xlsx",
  overwrite = T
)
obs.res_VegDiet_bin <-
  read.xlsx("results/ALSPAC/IMP_PNC_obs.res_VegDiet_bin.xlsx")
...
head(obs.res_VegDiet_bin)
dim(obs.res_VegDiet_bin)  # 84 -> 82 obs
################################################################################
...
ggsave(
  obs.forest_VegDiet_bin,
  file = "results/ALSPAC/IMP_PNC_obs.forest_VegDiet_bin.png",
  height = 18,
  width = 7
)
...
for (myoutcome in ALSPAC_out_con) {
  ...
  #### Model 2 (maternal model adjusted for paternal exposure): VegDiet_bin + age, education, IMD, parity, BMI, smoking, alcohol, offspring sex + VietDiet_3cat_Pat
  mod_2 <- list()
  for (i in 1:dat_imp$m) {
    mod_2[[i]] <-
      lm(
  ...
  #### Model 3 (paternal model): VegDiet_bin_Pat + age, education, IMD, number of children, BMI, smoking, alcohol, offspring sex
  mod_3 <- list()
  for (i in 1:dat_imp$m) {
    mod_3[[i]] <-
      lm(dat[[i]][[myoutcome]] ~ dat[[i]][["VegDiet_bin_Pat"]] + dat[[i]][["age_Pat_con"]] + dat[[i]][["ethnic_Pat_bin"]] + dat[[i]][["edu_Pat_3cat"]]  + dat[[i]][["IMD_Fam_cat"]] + dat[[i]][["parity_Pat_bin"]] + dat[[i]][["BMI_Pat_EAR.p_con"]] + dat[[i]][["smoking_Pat_EAR.p_bin"]] + dat[[i]][["alcohol_Pat_EAR.p_bin"]] + dat[[i]][["sex_Chi_bin"]])
  }
  mod_3_pooled <- pool(mod_3)
  x_3 <- IMP_extract_lin.res_bin(mod_3_pooled)
  y_3 <- IMP_extract_beta.SE_bin(mod_3_pooled)
  
  #### Model 4 (paternal model adjusted for maternal exposure): VegDiet_bin_Pat + age, education, IMD, number of children, BMI, smoking, alcohol, offspring sex + VietDiet_3cat
  mod_4 <- list()
  for (i in 1:dat_imp$m) {
    mod_4[[i]] <-
      lm(
...
write.xlsx(
  obs.tbl_VegDiet_con,
  "results/ALSPAC/IMP_PNC_obs.tbl_VegDiet_con.xlsx",
  overwrite = T
)
obs.tbl_VegDiet_con <-
  read.xlsx("results/ALSPAC/IMP_PNC_obs.tbl_VegDiet_con.xlsx")
...
write.xlsx(
  obs.res_VegDiet_con,
  "results/ALSPAC/IMP_PNC_obs.res_VegDiet_con.xlsx",
  overwrite = T
)
obs.res_VegDiet_con <-
  read.xlsx("results/ALSPAC/IMP_PNC_obs.res_VegDiet_con.xlsx")
...
ggsave(
  obs.forest_VegDiet_con,
  file = "results/ALSPAC/IMP_PNC_obs.forest_VegDiet_con.png",
  height = 5,
  width = 6
)
