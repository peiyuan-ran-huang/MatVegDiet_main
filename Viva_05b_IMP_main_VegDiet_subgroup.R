...
# This script is to perform main association analysis (with imputed data) for vegetarian diets (in subgroups) in Project Viva.
...
obs.tbl_VegDiet_bin
dim(obs.tbl_VegDiet_bin) # 17 outcomes * 3 categories = 51 obs in 3 models
write.xlsx(
  obs.tbl_VegDiet_bin,
  "results/Viva/IMP_MAIN_obs.tbl_VegDiet.subgroup_bin.xlsx",
  overwrite = T
)
obs.tbl_VegDiet_bin <-
  read.xlsx("results/Viva/IMP_MAIN_obs.tbl_VegDiet.subgroup_bin.xlsx")
...
obs.res_VegDiet_bin
dim(obs.res_VegDiet_bin)  # 17 outcomes * 2 categories * 3 models = 102 obs.
write.xlsx(
  obs.res_VegDiet_bin,
  "results/Viva/IMP_MAIN_obs.res_VegDiet.subgroup_bin.xlsx",
  overwrite = T
)
obs.res_VegDiet_bin <-
  read.xlsx("results/Viva/IMP_MAIN_obs.res_VegDiet.subgroup_bin.xlsx")
...
dim(obs.res_VegDiet_bin)  # 17 outcomes * 2 categories * 1 model = 34 obs.
...
head(obs.res_VegDiet_bin)
dim(obs.res_VegDiet_bin)  # 34 -> 22 obs.
################################################################################
...
ggsave(
  obs.forest_VegDiet_bin,
  file = "results/Viva/IMP_MAIN_obs.forest_VegDiet.subgroup_bin.png",
  height = 7,
  width = 7
)
...
IMP_extract_lin.res_3cat <- function(mymodel) {
  ...
  ### Full vegetarian
  Exposure_3 <- "Full vegetarian"
  Beta_3 <- format(round(summary(mymodel)[3, 2], digits = 2), nsmall = 2)
...
for (myoutcome in Viva_out_con) {
  ...
  N_x <- as.data.frame(c(nrow(dat_1[dat_1$VegDiet_3cat == "Non-vegetarian" &
...
obs.tbl_VegDiet_con
dim(obs.tbl_VegDiet_con)  # 2 outcomes for 3 categories in 3 models = 6 obs.
write.xlsx(
  obs.tbl_VegDiet_con,
  "results/Viva/IMP_MAIN_obs.tbl_VegDiet.subgroup_con.xlsx",
  overwrite = T
)
obs.tbl_VegDiet_con <-
  read.xlsx("results/Viva/IMP_MAIN_obs.tbl_VegDiet.subgroup_con.xlsx")
...
obs.res_VegDiet_con
dim(obs.res_VegDiet_con)  # 2 outcomes * 2 categories * 3 models = 12 obs.
write.xlsx(
  obs.res_VegDiet_con,
  "results/Viva/IMP_MAIN_obs.res_VegDiet.subgroup_con.xlsx",
  overwrite = T
)
obs.res_VegDiet_con <-
  read.xlsx("results/Viva/IMP_MAIN_obs.res_VegDiet.subgroup_con.xlsx")
...
dim(obs.res_VegDiet_con)  # 2 outcomes * 2 categories * 1 model = 4 obs.
...
ggsave(
  obs.forest_VegDiet_con,
  file = "results/Viva/IMP_MAIN_obs.forest_VegDiet.subgroup_con.png",
  height = 3,
  width = 7
)
