...
# This script is to examine socioeconomic position (SEP) on both vegetarianism (in subgroups) and perinatal outcomes (with imputed data) in MoBa.
...
write.xlsx(
  obs.res_VegDiet_bin_con,
  "results/IMP_SEP_obs.res_VegDiet.subgroup_bin_con.xlsx",
  overwrite = T
)
obs.res_VegDiet_bin_con <-
  read.xlsx("results/IMP_SEP_obs.res_VegDiet.subgroup_bin_con.xlsx")
...
colnames(obs.res_VegDiet_bin_con)[colnames(obs.res_VegDiet_bin_con) == "SEP"] <-
  "SEP indicator"  # Change the column name to be shown in the forest plot
...
obs.forest_VegDiet_bin_con <- ggforestplot::forestplot(
  ...
  title = "Effect of socioeconomic position (SEP) in MoBa",
  logodds = F
) +
...
ggsave(
  obs.forest_VegDiet_bin_con,
  file = "results/IMP_SEP_obs.forest_VegDiet.subgroup_bin_con.png",
  height = 10,
  width = 10
)
