...
# Last edited date: 14-Jun-2024
# This script is to perform sensitivity analyses (with imputed data) for vegetarian diets in Project Viva.
## Sensitivity Analysis 3 - Dietary exposure in mid-pregnancy
...
for (i in 1:dat_imp$m) {
  ...
  dat[[i]] <- complete_dat
  dat[[i]] <- subset(dat[[i]], !is.na(dat[[i]]$VegDiet_bin_2))  # Keep only those with dietary exposure data in early pregnancy
}
...
MRPREG_outcome_labels
dim(MRPREG_outcome_labels)  # 60 MR-PREG outcomes in total
...
## Model 2: + age, ethnicity, education, household income, parity, pre-pregnancy BMI, smoking, alcohol drinking, offspring sex - i.e., additionally adjusting for sociodemographic, pregnancy-related, and lifestyle factors
...
IMP_extract_log.res_bin <- function(mymodel) {
  ...
  OR_2 <- format(round(exp(summary(mymodel)[2, 2]), digits = 2), nsmall = 2)
...
for (myoutcome in Viva_out_bin) {
  ...
  for (i in 1:dat_imp$m) {
    mod_1[[i]] <-
      glm(dat[[i]][[myoutcome]] ~ dat[[i]][["VegDiet_bin_2"]], family = binomial)
  }
  ...
  for (i in 1:dat_imp$m) {
    mod_2[[i]] <-
      glm(dat[[i]][[myoutcome]] ~ dat[[i]][["VegDiet_bin_2"]] + dat[[i]][["age_Mat_con"]] + dat[[i]][["ethnic_Mat_bin"]] + dat[[i]][["edu_Mat_3cat"]]  + dat[[i]][["income_Fam_3cat"]] + dat[[i]][["parity_Mat_bin"]] + dat[[i]][["BMI_Mat_PRE.p_con"]] + dat[[i]][["smoking_Mat_EAR.p_bin"]] + dat[[i]][["alcohol_Mat_EAR.p_bin"]] + dat[[i]][["sex_Chi_bin"]],
          family = binomial)
  }
  ...
  for (i in 1:dat_imp$m) {
    mod_3[[i]] <-
      glm(
        dat[[i]][[myoutcome]] ~ dat[[i]][["VegDiet_bin_2"]] + dat[[i]][["age_Mat_con"]] + dat[[i]][["ethnic_Mat_bin"]] + dat[[i]][["edu_Mat_3cat"]]  + dat[[i]][["income_Fam_3cat"]] + dat[[i]][["parity_Mat_bin"]] + dat[[i]][["BMI_Mat_PRE.p_con"]] + dat[[i]][["smoking_Mat_EAR.p_bin"]] + dat[[i]][["alcohol_Mat_EAR.p_bin"]] + dat[[i]][["sex_Chi_bin"]] + dat[[i]][["any.supp_Mat_EAR.p_bin"]] + dat[[i]][["energy_Mat_EAR.p_con"]] + dat[[i]][["hPDIm_2"]],
        family = binomial
      )
  }
  ...
  N_x <- as.data.frame(c(paste0(nrow(dat_1[dat_1$VegDiet_bin_2 == 0 &
                                             !is.na(dat_1[[myoutcome]]) &
                                             dat_1[[myoutcome]] == 1, ]), " / ", nrow(dat_1[dat_1$VegDiet_bin_2 == 0 &
                                                                                              !is.na(dat_1[[myoutcome]]), ])), paste0(nrow(dat_1[dat_1$VegDiet_bin_2 == 1 &
                                                                                                                                                   !is.na(dat_1[[myoutcome]]) &
                                                                                                                                                   dat_1[[myoutcome]] == 1, ]), " / ", nrow(dat_1[dat_1$VegDiet_bin_2 == 1 &
                                                                                                                                                                                                    !is.na(dat_1[[myoutcome]]), ]))))
  colnames(N_x) <- "N"
...
obs.tbl_VegDiet_bin[, c(7, 11)] <- " " # For convenience when making tables
...
obs.tbl_VegDiet_bin <- obs.tbl_VegDiet_bin[, c(1, 3, 2, 4:ncol(obs.tbl_VegDiet_bin))]  # For convenience when making tables
obs.tbl_VegDiet_bin
dim(obs.tbl_VegDiet_bin) # 17 outcomes * 2 categories = 34 obs in 3 models
write.xlsx(
  obs.tbl_VegDiet_bin,
  "results/Viva/IMP_SENS.MID.p_obs.tbl_VegDiet_bin.xlsx",
  overwrite = T
)
obs.tbl_VegDiet_bin <-
  read.xlsx("results/Viva/IMP_SENS.MID.p_obs.tbl_VegDiet_bin.xlsx")
...
write.xlsx(
  obs.res_VegDiet_bin,
  "results/Viva/IMP_SENS.MID.p_obs.res_VegDiet_bin.xlsx",
  overwrite = T
)
obs.res_VegDiet_bin <-
  read.xlsx("results/Viva/IMP_SENS.MID.p_obs.res_VegDiet_bin.xlsx")
...
head(obs.res_VegDiet_bin)
dim(obs.res_VegDiet_bin)  # 17 -> 12 obs.
################################################################################
...
ggsave(
  obs.forest_VegDiet_bin,
  file = "results/Viva/IMP_SENS.MID.p_obs.forest_VegDiet_bin.png",
  height = 4,
  width = 7
)
...
IMP_extract_lin.res_3cat <- function(mymodel) {
  ...
  ### Pesco-/full vegetarian
  Exposure_2 <- "Pesco-/full vegetarian"
  Beta_2 <- format(round(summary(mymodel)[2, 2], digits = 2), nsmall = 2)
...
for (myoutcome in Viva_out_con) {
  ...
  mod_1 <- list()
  for (i in 1:dat_imp$m) {
    mod_1[[i]] <- lm(dat[[i]][[myoutcome]] ~ dat[[i]][["VegDiet_bin_2"]])
  }
  mod_1_pooled <- pool(mod_1)
  ...
  for (i in 1:dat_imp$m) {
    mod_2[[i]] <-
      lm(dat[[i]][[myoutcome]] ~ dat[[i]][["VegDiet_bin_2"]] + dat[[i]][["age_Mat_con"]] + dat[[i]][["ethnic_Mat_bin"]] + dat[[i]][["edu_Mat_3cat"]] + dat[[i]][["income_Fam_3cat"]] + dat[[i]][["parity_Mat_bin"]] + dat[[i]][["BMI_Mat_PRE.p_con"]] + dat[[i]][["smoking_Mat_EAR.p_bin"]] + dat[[i]][["alcohol_Mat_EAR.p_bin"]] + dat[[i]][["sex_Chi_bin"]])
  }
  mod_2_pooled <- pool(mod_2)
  ...
  N_x <- as.data.frame(c(nrow(dat_1[dat_1$VegDiet_bin_2 == 0 &
                                      !is.na(dat_1[[myoutcome]]), ]), nrow(dat_1[dat_1$VegDiet_bin_2 == 1 &
                                                                                   !is.na(dat_1[[myoutcome]]), ])))
  colnames(N_x) <- "N"
...
obs.tbl_VegDiet_con[, c(7, 11)] <- " "  # For convenience when making tables
obs.tbl_VegDiet_con <- obs.tbl_VegDiet_con[, c(1, 3, 2, 4:ncol(obs.tbl_VegDiet_con))]  # For convenience when making tables
obs.tbl_VegDiet_con
dim(obs.tbl_VegDiet_con)  # 2 outcomes for 2 categories in 3 models = 4 obs.
write.xlsx(
  obs.tbl_VegDiet_con,
  "results/Viva/IMP_SENS.MID.p_obs.tbl_VegDiet_con.xlsx",
  overwrite = T
)
obs.tbl_VegDiet_con <-
  read.xlsx("results/Viva/IMP_SENS.MID.p_obs.tbl_VegDiet_con.xlsx")
...
write.xlsx(
  obs.res_VegDiet_con,
  "results/Viva/IMP_SENS.MID.p_obs.res_VegDiet_con.xlsx",
  overwrite = T
)
obs.res_VegDiet_con <-
  read.xlsx("results/Viva/IMP_SENS.MID.p_obs.res_VegDiet_con.xlsx")
...
ggsave(
  obs.forest_VegDiet_con,
  file = "results/Viva/IMP_SENS.MID.p_obs.forest_VegDiet_con.png",
  height = 2,
  width = 7
)
...
for (i in 1:dat_imp$m) {
  mod_1[[i]] <-
    clm(bf_dur_4c ~ VegDiet_bin_2, data = dat[[i]])
}
mod_1_pooled <- pool(mod_1)
...
for (i in 1:dat_imp$m) {
  mod_2[[i]] <-
    clm(
      bf_dur_4c ~ VegDiet_bin_2 + age_Mat_con + ethnic_Mat_bin + edu_Mat_3cat  + income_Fam_3cat + parity_Mat_bin + BMI_Mat_PRE.p_con + smoking_Mat_EAR.p_bin + alcohol_Mat_EAR.p_bin + sex_Chi_bin,
      data = dat[[i]]
    )
}
mod_2_pooled <- pool(mod_2)
...
for (i in 1:dat_imp$m) {
  mod_3[[i]] <-
    clm(
      bf_dur_4c ~ VegDiet_bin_2 + age_Mat_con + ethnic_Mat_bin + edu_Mat_3cat  + income_Fam_3cat + parity_Mat_bin + BMI_Mat_PRE.p_con + smoking_Mat_EAR.p_bin + alcohol_Mat_EAR.p_bin + sex_Chi_bin + any.supp_Mat_EAR.p_bin + hPDIm_2,
      data = dat[[i]]
    )
}
mod_3_pooled <- pool(mod_3)
...
colnames(outcome_x) <- "Outcome"
N_x <-
  as.data.frame(c(nrow(dat_1[dat_1$VegDiet_bin_2 == 0 &
                               !is.na(dat_1$bf_dur_4c), ]), nrow(dat_1[dat_1$VegDiet_bin_2 == 1 &
...
write.xlsx(
  obs.tbl_VegDiet_ord,
  "results/Viva/IMP_SENS.MID.p_obs.tbl_VegDiet_ord.xlsx",
  overwrite = T
)
obs.tbl_VegDiet_ord <-
  read.xlsx("results/Viva/IMP_SENS.MID.p_obs.tbl_VegDiet_ord.xlsx")
...
write.xlsx(
  obs.res_VegDiet_ord,
  "results/Viva/IMP_SENS.MID.p_obs.res_VegDiet_ord.xlsx",
  overwrite = T
)
obs.res_VegDiet_ord <-
  read.xlsx("results/Viva/IMP_SENS.MID.p_obs.res_VegDiet_ord.xlsx")
