...
# This script is to perform stratification analyses (with imputed data) for vegetarian diets in ALSPAC.
...
## Subset samples
dat_N_iron <-
  subset(dat_1, iron.supp_Mat_EAR.p_bin == 0)
dat_iron <-
  subset(dat_1, iron.supp_Mat_EAR.p_bin == 1)
...
for (i in 1:dat_imp$m) {
  mod_N_iron[[i]] <-
    glm(
      anaemia_preg_all ~ VegDiet_bin + age_Mat_con + ethnic_Mat_bin + edu_Mat_3cat  + IMD_Fam_cat + parity_Mat_bin + BMI_Mat_PRE.p_con + smoking_Mat_EAR.p_bin + alcohol_Mat_EAR.p_bin + sex_Chi_bin + NON.iron.supp_Mat_EAR.p_bin + energy_Mat_DUR.p_con + hPDIm,
      data = subset(dat[[i]], iron.supp_Mat_EAR.p_bin == 0),
      family = binomial
    )
}
...
x_N_iron <-
  as.data.frame(IMP_extract_log.res_bin(mod_N_iron_pooled))
y_N_iron <-
  IMP_extract_beta.SE_bin(mod_N_iron_pooled)
...
for (i in 1:nrow(y_N_iron)) {
  y_N_iron$N_exp[i] <- paste0(
    sum(
      dat_N_iron$VegDiet_bin == 1 &
        dat_N_iron$anaemia_preg_all == 1,
      na.rm = T
    ),
    " / ",
    sum(dat_N_iron$VegDiet_bin == 1, na.rm = T)
  )
  
  y_N_iron$N_ref[i] <- paste0(
    sum(
      dat_N_iron$VegDiet_bin == 0 &
        dat_N_iron$anaemia_preg_all == 1,
      na.rm = T
    ),
    " / ",
    sum(dat_N_iron$VegDiet_bin == 0, na.rm = T)
  )
}
...
for (i in 1:dat_imp$m) {
  mod_iron[[i]] <-
    glm(
      anaemia_preg_all ~ VegDiet_bin + age_Mat_con + ethnic_Mat_bin + edu_Mat_3cat  + IMD_Fam_cat + parity_Mat_bin + BMI_Mat_PRE.p_con + smoking_Mat_EAR.p_bin + alcohol_Mat_EAR.p_bin + sex_Chi_bin + NON.iron.supp_Mat_EAR.p_bin + energy_Mat_DUR.p_con + hPDIm,
      data = subset(dat[[i]], iron.supp_Mat_EAR.p_bin == 1),
      family = binomial
    )
}
...
x_iron <-
  as.data.frame(IMP_extract_log.res_bin(mod_iron_pooled))
y_iron <-
  IMP_extract_beta.SE_bin(mod_iron_pooled)
...
for (i in 1:nrow(y_iron)) {
  y_iron$N_exp[i] <- paste0(
    sum(
      dat_iron$VegDiet_bin == 1 &
        dat_iron$anaemia_preg_all == 1,
      na.rm = T
    ),
    " / ",
    sum(dat_iron$VegDiet_bin == 1, na.rm = T)
  )
  
  y_iron$N_ref[i] <- paste0(
    sum(
      dat_iron$VegDiet_bin == 0 &
        dat_iron$anaemia_preg_all == 1,
      na.rm = T
    ),
    " / ",
    sum(dat_iron$VegDiet_bin == 0, na.rm = T)
  )
}
...
write.xlsx(
  obs.res_VegDiet_bin,
  "results/ALSPAC/IMP_STRAT.iron_obs.res_VegDiet_bin.xlsx",
  overwrite = T
)
obs.res_VegDiet_bin <-
  read.xlsx("results/ALSPAC/IMP_STRAT.iron_obs.res_VegDiet_bin.xlsx")
...
obs.forest_VegDiet_bin <- ggforestplot::forestplot(
  ...
  title = "Maternal anaemia in ALSPAC",
  logodds = T
) +
...
ggsave(
  obs.forest_VegDiet_bin,
  file = "results/ALSPAC/IMP_STRAT.iron_obs.forest_VegDiet_bin.png",
  height = 2,
  width = 6
)
