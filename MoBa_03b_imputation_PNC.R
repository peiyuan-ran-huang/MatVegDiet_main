...
# This script is to perform multiple imputation by chained equations (MICE) for paternal negative control analysis in MoBa.
...
dat <- subset(dat, is.na(VegDiet_3cat_Pat) == F)
head(dat)
dim(dat)  # 73868 -> 21952
#!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
################################################################################
...
substantive_var <-
  read.xlsx("select_var.xlsx", sheet = "Substantive_Pat")
...
auxiliary_var <-
  read.xlsx("select_var.xlsx", sheet = "Auxiliary_Pat")
auxiliary_var  # Auxiliary variables (e.g., IDs, exposures, maternal covariates)
mean(auxiliary_var$var_name %in% colnames(dat)) == 1  # Check if all variables are available in the dataset

################################################################################
other_imp <-
  c()  # Other variables to be imputed (e.g., used for adjustment)
other_N_imp <-
  c("VegDiet_bin", "VegDiet_bin_Pat")  # Other variables NOT to be imputed (e.g., used as alternative exposure)
################################################################################
...
## Check missing data
sum(is.na(dat$PREG_ID_2552))  # 0
sum(is.na(dat$VegDiet_3cat))  # 0
sum(is.na(dat$VegDiet_bin))  # 0
sum(is.na(dat$VegDiet_3cat_Pat))  # 0
sum(is.na(dat$VegDiet_bin_Pat))  # 0
...
cov <-
  substantive_var$var_name  # Specify covariates (i.e., key variables to be imputed)
cov
# [1] "age_Pat_con"            "edu_Pat_3cat"           "income_Fam_3cat"        "parity_Pat_bin"
# [5] "BMI_Pat_EAR.p_con"      "smoking_Pat_EAR.p_bin"  "alcohol_Pat_EAR.p_bin"  "any.supp_Pat_EAR.p_bin"
# [9] "sex_Chi_bin"
...
head(dat_cov)
dim(dat_cov)  # 21922    9 (= 12 - 3; without: ethnicity, energy intake, hPDI)
...
(num_incomplete_cases / num_total_cases) * 100  # Percentage of incomplete cases: 18.02755%
...
## Save imputed data
save(dat_imp, file = "dat_exp_cov_out_pat_IMP_PNC.RData")
...
tab_ori_imp <- tbl_summary(
  ...
  modify_caption(
    "Distribution of covariates (for paternal negative control analysis) in the observed and imputed datasets in MoBa"
  ) %>%
  bold_labels() %>%
  modify_table_body(~ .x %>%
                      dplyr::mutate(across(
...
tab_ori_imp %>% as_flex_table() %>% flextable::save_as_docx(., path =
                                                              "results/00b-paternal_covariates_OBS.vs.IMP.docx")
