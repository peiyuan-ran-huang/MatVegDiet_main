...
# Part 2: For interpretation (combined with confounding structure)
...
metagen_obs.tbl_VegDiet_ord
str(metagen_obs.tbl_VegDiet_ord)  # 2 obs. = 1 outcome * 2 exposures

write.xlsx(
  metagen_obs.tbl_VegDiet_ord,
  "ALL/IMP_BF_meta.tbl_VegDiet.subgroup_ord.xlsx",
  rowNames = F
)
...
png(
  "ALL/IMP_BF_meta.forest_VegDiet.subgroup_ord.png",
  res = 300,
  height = 1200,
  width = 2300
)
...
### ALSPAC
obs.res_VegDiet_SEP_ALSPAC <- read.xlsx("ALSPAC/IMP_BF_obs.res_VegDiet.subgroup_SEP.xlsx")
obs.res_VegDiet_SEP_ALSPAC$Study <- "ALSPAC"
obs.res_VegDiet_SEP_ALSPAC$Outcome[obs.res_VegDiet_SEP_ALSPAC$Outcome ==
                                     "Pesco-vegetarianism"] <- "Pesco- vs. non-vegetarian"
obs.res_VegDiet_SEP_ALSPAC$Outcome[obs.res_VegDiet_SEP_ALSPAC$Outcome ==
                                     "Full vegetarianism"] <- "Full vs. non-vegetarian"
obs.res_VegDiet_SEP_ALSPAC$Outcome[obs.res_VegDiet_SEP_ALSPAC$Outcome ==
...
### BiB
obs.res_VegDiet_SEP_BiB <- read.xlsx("BiB/IMP_BF_obs.res_VegDiet.subgroup_SEP.xlsx")
...
### MoBa
obs.res_VegDiet_SEP_MoBa <- read.xlsx("MoBa/IMP_BF_obs.res_VegDiet.subgroup_SEP.xlsx")
...
### Project Viva
obs.res_VegDiet_SEP_Viva <- read.xlsx("Viva/IMP_BF_obs.res_VegDiet.subgroup_SEP.xlsx")
...
ggsave(
  obs.forest_VegDiet_SEP,
  file = "ALL/IMP_BF_obs.forest_VegDiet.subgroup_SEP_ALL.png",
  height = 6,
  width = 12
)
...
title2 <- textGrob(
  "Effect of socioeconimic position (SEP) on maternal vegetarian diet adherence and breastfeeding duration",
  gp = gpar(fontsize = 12, fontface = "bold")
)
...
png(
  "ALL/Comb_IMP_BF.meta.forest_SEP.obs.forest_VegDiet.subgroup.png",
  res = 300,
  height = 3500,
  width = 3000
)
