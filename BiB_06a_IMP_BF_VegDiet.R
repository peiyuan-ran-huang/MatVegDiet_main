...
# This script is to perform negative control outcome analysis on breastfeeding (with imputed data) for vegetarian diets in BiB.
...
for (i in 1:dat_imp$m) {
  mod_3[[i]] <-
    clm(
      bf_dur_4c ~ VegDiet_bin + age_Mat_con + ethnic_Mat_bin + edu_Mat_3cat  + IMD_Fam_cat + parity_Mat_bin + BMI_Mat_PRE.p_con + smoking_Mat_EAR.p_bin + alcohol_Mat_EAR.p_bin + sex_Chi_bin + any.supp_Mat_EAR.p_bin,
      data = dat[[i]]
    )
}
...
write.xlsx(obs.tbl_VegDiet_ord,
           "results/BiB/IMP_BF_obs.tbl_VegDiet_ord.xlsx",
           overwrite = T)
obs.tbl_VegDiet_ord <-
  read.xlsx("results/BiB/IMP_BF_obs.tbl_VegDiet_ord.xlsx")
...
write.xlsx(obs.res_VegDiet_ord,
           "results/BiB/IMP_BF_obs.res_VegDiet_ord.xlsx",
           overwrite = T)
obs.res_VegDiet_ord <-
  read.xlsx("results/BiB/IMP_BF_obs.res_VegDiet_ord.xlsx")
...
write.xlsx(obs.res_VegDiet_SEP,
           "results/BiB/IMP_BF_obs.res_VegDiet_SEP.xlsx",
           overwrite = T)
obs.res_VegDiet_SEP <-
  read.xlsx("results/BiB/IMP_BF_obs.res_VegDiet_SEP.xlsx")
...
ggsave(
  obs.forest_VegDiet_SEP,
  file = "results/BiB/IMP_BF_obs.forest_VegDiet_SEP.png",
  height = 2,
  width = 9
)
