...
# This script is to perform main association analysis for vegetarian diets in ALSPAC.
...
for (myoutcome in ALSPAC_out_bin) {
  ...
  mod_3 <-
    glm(
      dat[[myoutcome]] ~ dat[["VegDiet_bin"]] + dat[["age_Mat_con"]] + dat[["ethnic_Mat_bin"]] + dat[["edu_Mat_3cat"]]  + dat[["IMD_Fam_cat"]] + dat[["parity_Mat_bin"]] + dat[["BMI_Mat_PRE.p_con"]] + dat[["smoking_Mat_EAR.p_bin"]] + dat[["alcohol_Mat_EAR.p_bin"]] + dat[["sex_Chi_bin"]] + dat[["any.supp_Mat_EAR.p_bin"]] + dat[["energy_Mat_DUR.p_con"]] + dat[["hPDIm"]],
      family = binomial
    )
...
write.xlsx(obs.tbl_VegDiet_bin,
           "results/ALSPAC/MAIN_obs.tbl_VegDiet_bin.xlsx",
           overwrite = T)
obs.tbl_VegDiet_bin <-
  read.xlsx("results/ALSPAC/MAIN_obs.tbl_VegDiet_bin.xlsx")
...
write.xlsx(obs.res_VegDiet_bin,
           "results/ALSPAC/MAIN_obs.res_VegDiet_bin.xlsx",
           overwrite = T)
obs.res_VegDiet_bin <-
  read.xlsx("results/ALSPAC/MAIN_obs.res_VegDiet_bin.xlsx")
...
head(obs.res_VegDiet_bin)
dim(obs.res_VegDiet_bin)  # 21 -> 20 obs
################################################################################
...
ggsave(
  obs.forest_VegDiet_bin,
  file = "results/ALSPAC/MAIN_obs.forest_VegDiet_bin.png",
  height = 5,
  width = 8
)
...
for (myoutcome in ALSPAC_out_con) {
  ...
  #### Model 3 - Additionally adjusting for nutrition-related factors (including hPDIm as the indicator for the healthfulness of diet)
  mod_3 <-
    lm(dat[[myoutcome]] ~ dat[["VegDiet_bin"]] + dat[["age_Mat_con"]] + dat[["ethnic_Mat_bin"]] + dat[["edu_Mat_3cat"]]  + dat[["IMD_Fam_cat"]] + dat[["parity_Mat_bin"]] + dat[["BMI_Mat_PRE.p_con"]] + dat[["smoking_Mat_EAR.p_bin"]] + dat[["alcohol_Mat_EAR.p_bin"]] + dat[["sex_Chi_bin"]] + dat[["any.supp_Mat_EAR.p_bin"]] + dat[["energy_Mat_DUR.p_con"]] + dat[["hPDIm"]])
...
write.xlsx(obs.tbl_VegDiet_con,
           "results/ALSPAC/MAIN_obs.tbl_VegDiet_con.xlsx",
           overwrite = T)
obs.tbl_VegDiet_con <-
  read.xlsx("results/ALSPAC/MAIN_obs.tbl_VegDiet_con.xlsx")
...
write.xlsx(obs.res_VegDiet_con,
           "results/ALSPAC/MAIN_obs.res_VegDiet_con.xlsx",
           overwrite = T)
obs.res_VegDiet_con <-
  read.xlsx("results/ALSPAC/MAIN_obs.res_VegDiet_con.xlsx")
...
ggsave(
  obs.forest_VegDiet_con,
  file = "results/ALSPAC/MAIN_obs.forest_VegDiet_con.png",
  height = 2,
  width = 6
)
