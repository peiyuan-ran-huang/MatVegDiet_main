...
# This script is to perform multiple imputation by chained equations (MICE) for paternal negative control analysis in ALSPAC.
...
substantive_var <-
  read.xlsx("data/ALSPAC/select_var.xlsx", sheet = "Substantive_Pat")
...
auxiliary_var <-
  read.xlsx("data/ALSPAC/select_var.xlsx", sheet = "Auxiliary_Pat")
...
################################################################################
other_imp <- c()  # Other variables to be imputed
other_N_imp <- c("VegDiet_bin", "VegDiet_bin_Pat")  # Other variables NOT to be imputed (e.g., used as alternative exposure)
################################################################################
...
cov
# [1] "age_Pat_con"           "ethnic_Pat_bin"        "edu_Pat_3cat"          "IMD_Fam_cat"
# [5] "parity_Pat_bin"        "BMI_Pat_EAR.p_con"     "smoking_Pat_EAR.p_bin" "alcohol_Pat_EAR.p_bin"
# [9] "sex_Chi_bin"
...
head(dat_cov)
dim(dat_cov)  # 8557    9 (= 12 - 3; without: supplementation, energy intake, hPDI)
...
(num_incomplete_cases / num_total_cases) * 100  # Percentage of incomplete cases: 31.95335%
...
## Save imputed data
save(dat_imp, file = "data/ALSPAC/dat_exp_cov_out_pat_IMP_PNC.RData")
...
tab_ori_imp <- tbl_summary(
  ...
  modify_caption(
    "Distribution of covariates (for paternal negative control analysis) in the observed and imputed datasets in ALSPAC"
  ) %>%
  bold_labels() %>%
  modify_table_body(~ .x %>%
                      dplyr::mutate(across(
...
tab_ori_imp %>% as_flex_table() %>% flextable::save_as_docx(., path =
                                                              "results/ALSPAC/00b-paternal_covariates_OBS.vs.IMP.docx")
