...
# This script is to perform multiple imputation by chained equations (MICE) for main association analysis in MoBa.
...
substantive_var <-
  read.xlsx("select_var.xlsx", sheet = "Substantive")
...
auxiliary_var <-
  read.xlsx("select_var.xlsx", sheet = "Auxiliary")
...
################################################################################
other_imp <-
  c("NON.iron.supp_Mat_EAR.p_bin",
    "NON.vitB12.supp_Mat_EAR.p_bin")  # Other variables to be imputed (e.g., used for adjustment)
other_N_imp <-
  c(
    "iron.supp_Mat_EAR.p_bin",
    "iron.supp.dur_Mat_EAR.p_cat",
    "iron.supp.freq_Mat_EAR.p_cat",
    "vitB12.supp_Mat_EAR.p_bin",
    "VegDiet_bin",
    "VegDiet_3cat_FFQ_self",
...
cov
# [1] "age_Mat_con"            "ethnic_Mat_bin"         "edu_Mat_3cat"           "income_Fam_3cat"
...
(num_incomplete_cases / num_total_cases) * 100  # Percentage of incomplete cases: 26.25052%
...
## Save imputed data
save(dat_imp, file = "dat_exp_cov_out_pat_IMP_main.RData")
...
tab_ori_imp <- tbl_summary(
  ...
  modify_caption(
    "Distribution of covariates (for main analysis) in the observed and imputed datasets in MoBa"
  ) %>%
  bold_labels() %>%
  modify_table_body(~ .x %>%
                      dplyr::mutate(across(
...
tab_ori_imp %>% as_flex_table() %>% flextable::save_as_docx(., path =
                                                              "results/00a-maternal_covariates_OBS.vs.IMP.docx")
