...
# This script is to perform negative control outcome analysis on breastfeeding for vegetarian diets in MoBa.
...
write.xlsx(obs.tbl_VegDiet_ord,
           "results/BF_obs.tbl_VegDiet_ord.xlsx",
           overwrite = T)
obs.tbl_VegDiet_ord <-
  read.xlsx("results/BF_obs.tbl_VegDiet_ord.xlsx")
...
write.xlsx(obs.res_VegDiet_ord,
           "results/BF_obs.res_VegDiet_ord.xlsx",
           overwrite = T)
obs.res_VegDiet_ord <-
  read.xlsx("results/BF_obs.res_VegDiet_ord.xlsx")
...
write.xlsx(obs.res_VegDiet_SEP,
           "results/BF_obs.res_VegDiet_SEP.xlsx",
           overwrite = T)
obs.res_VegDiet_SEP <-
  read.xlsx("results/BF_obs.res_VegDiet_SEP.xlsx")
...
ggsave(
  obs.forest_VegDiet_SEP,
  file = "results/BF_obs.forest_VegDiet_SEP.png",
  height = 2,
  width = 9
)
...
mod_3 <-
  clm(bf_dur_4c ~ VegDiet_bin + income_Fam_3cat, data = dat)  # Adjusted for income only
as.data.frame(extract_ord.log.res_bin(mod_3))

mod_4 <-
  clm(bf_dur_4c ~ VegDiet_bin + edu_Mat_3cat + income_Fam_3cat, data = dat)  # Adjusted for both maternal education and income
