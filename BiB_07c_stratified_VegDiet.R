...
# This script is to perform stratification analyses for vegetarian diets in BiB.
...
## Subset samples
dat_N_iron <-
  subset(dat, iron.multivit.supp_Mat_EAR.p_bin == "No")
dat_iron <-
  subset(dat, iron.multivit.supp_Mat_EAR.p_bin == "Yes")
...
for (i in 1:nrow(y_N_iron)) {
  y_N_iron$N_exp[i] <- x_N_iron$N[x_N_iron$Exposure == y_N_iron$Exposure[i]]
  y_N_iron$N_ref[i] <- x_N_iron$N[x_N_iron$Exposure == "Non-vegetarian (ref)"]
}
...
for (i in 1:nrow(y_iron)) {
  ...
  y_iron$N_ref[i] <- x_iron$N[x_iron$Exposure == "Non-vegetarian (ref)"]
}
...
write.xlsx(
  obs.res_VegDiet_bin,
  "results/BiB/STRAT.iron_obs.res_VegDiet_bin.xlsx",
  overwrite = T
)
obs.res_VegDiet_bin <-
  read.xlsx("results/BiB/STRAT.iron_obs.res_VegDiet_bin.xlsx")
...
ggsave(
  obs.forest_VegDiet_bin,
  file = "results/BiB/STRAT.iron_obs.forest_VegDiet_bin.png",
  height = 2,
  width = 6
)
